...
- **Layer 2 (Refine Context)**: task_context (→ context), optimizer_params — adjust when Layer 1 stalls
...
Fields: `critique_text` (formatted string), `thinking_styles`, `plan`, `context`, `optimizer_params`, `task_context`, `content_hashes`. The feedback cycle also carries `critique` (5-field dict: `positive_critique`, `negative_critique`, `priority_fix`, `suggested_axes`, `summary`) on `_LoopState` — fed to both L1 Generate and L2 Refine.
