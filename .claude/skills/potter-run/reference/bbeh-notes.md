...
**Resume via CLI works**: `run_optimization_notebook` auto-mints a session+cycle pair and claims `.promptpotter/active_session.json` with the same `CAMPAIGN_ARTIFACTS` + `SESSION_ARTIFACTS` as CLI fresh-mode `optimize --config …`. So `python -m promptpotter optimize` (no flags) resumes an interrupted notebook run via the active pointer. State reads happen by opening `campaigns/<cycle_id>/{dashboard.json,log.md,index.json}` directly — no CLI read commands.
...
Also: `l2_patience=5`, `l3_patience=3`, `creativity=0.7`, `improvement_threshold=0.01`, `seed=42`. The notebook injects `task_context` inline — for BBEH, no separate task-decompose step happens (and CLI fresh-mode `optimize --config …` won't auto-decompose because the BBEH dataset directory has no `task_description.md`).
