...
1. Resolve `cycle_id` from `.promptpotter/active_session.json`. Read in this order:
   ...
   - `campaigns/<cycle_id>/rounds/round_0001.json` — round 1 detail. (`round_0000.json` is origin.)
   - `campaigns/<cycle_id>/.cache/rounds/round_0001.json` — L1 variant payload (only if `review.md` flags a behaviour ✗ that needs the raw evidence).
...
3. Issue ranking — pick the **single** highest priority:
   1. Failed seeded check (`context_object_honored`, `param_scope_discipline`, `not_only_param_variants`)
