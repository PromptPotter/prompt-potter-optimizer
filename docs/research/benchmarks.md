...
Full table (11 methods) generated by `export_results supplemental`.

### Convergence, Ablation, Parameter Impact, Significance

Placeholder tables — filled after campaigns complete via `export_results supplemental`. Sections: per-round convergence (rounds 0-10), L1/L1+L2/full ablation, SearchMemory axis rankings, pairwise significance (McNemar + z-test with Bonferroni correction).
