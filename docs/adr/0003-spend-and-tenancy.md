...
Wire cost is extracted where the provider is spoken to — `infrastructure/llm/openai_compat.py::reply_cost` onto `LLMResponse.cost_usd` — and `call.py` passes it as the `cost_usd=` kwarg. An ADR states the decision; whether the code matches it is checked in the code.
