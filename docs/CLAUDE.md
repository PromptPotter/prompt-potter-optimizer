...
| [`developer/`](developer/) | Implementation specs. **[conventions](developer/conventions.md)** (style + code shape + the six reasoning doctrines) · [dispatch-hub](developer/dispatch-hub.md) (`INJECTIONS`, `L1Layout`, per-placeholder source map) · [adding-a-surface](developer/adding-a-surface.md) (a recipe per expansion point + the guard that catches each half-wiring) · [self-healing-internals](developer/self-healing-internals.md) · [node-standard](developer/node-standard.md) (the `pipeline.yaml` contract) · [stable-api](developer/stable-api.md) (the v1 fork-readiness surface) · [whitelabel](developer/whitelabel.md) · [dspy-optimizer](developer/dspy-optimizer.md). [`developer/README.md`](developer/README.md) is not an index — it owns the prompt-structure, dispatch, scoring-node and **§ 4 cross-run memory** sections. | this table |
| [`operations/`](operations/) | Running it — four unrelated subjects, load one group not the folder: **run/watch/diagnose** ([persistence-and-state](operations/persistence-and-state.md), whose § Diagnosing a live or stuck run — hang triage · [observability](operations/observability.md) · [backend-integration](operations/backend-integration.md)) · **datasets** ([dataset-selection-rationale](operations/dataset-selection-rationale.md), incl. § Adding a dataset · [dataset-reasoning-matrix](operations/dataset-reasoning-matrix.md)) · **security + deploy** ([access-model](operations/access-model.md) — the map an audit opens, incl. § What bounds resource use and § Running it securely · [`../deploy-linux/README.md`](../deploy-linux/README.md) for systemd + tunnel + OIDC) · **carry-over** ([mask-projection](operations/mask-projection.md)). Env vars are [`manual/02-install.md`](manual/02-install.md#environment-variables)'s. | this table |
| [`methods/`](methods/) | The statistical model + the two spend-control procedures that read it, split by the question each answers — **which samples** ([verdict-resolution](methods/verdict-resolution.md): the Rasch θ/δ model, the acquisition score behind `hard_samples.json`, the static within-round order) vs **which candidate** ([candidate-elimination](methods/candidate-elimination.md): PoBB's paired-sample fix, the θ stop rule, the five-mechanism ladder). | this table |
| [`research/`](research/) | Publication-facing: [benchmarks](research/benchmarks.md) (the origin-screening roster + the PEvol-Bench definition), [metrics](research/metrics.md) (Headroom Captured / Sample Efficiency / R₉₀), [related-work](research/related-work.md) (the algorithm-configuration umbrella, the classical AutoML lineage, the MCTS comparison), [bbeh-comparison/](research/bbeh-comparison/README.md) (head-to-head harness). | this table |
