...
**Status:** Slice 1 shipped (2026-05-05) — vanilla `webapp/index.html`, read-only, used in concert with the file tree. Slice 1 ≠ all of M11 webapp scope. Slice 1.5 hardened the vanilla file (a11y + tooltips + resilience) on 2026-05-07 (commit `f1dab82`) — it now serves as a fully-tested **migration preservation list** for the imminent Next.js port. Further M11 monitoring slices (hard-sample dashboard sibling to live samples, per-searchpoint score histogram across rounds, family-tree speciation view, dataset preview on drop) still belong in M11 and land **in the Next.js + React port** ([`m11-webapp-react-port.md`](m11-webapp-react-port.md)), not in further vanilla iteration. The vanilla file is now the migration preservation list, not an iteration target. The color-only status banner semantics (live / stale / offline) was fixed in vanilla's `/harden` pass — status dot now carries shape (filled / hollow ring / ×) in addition to color.
**Implementation:**
