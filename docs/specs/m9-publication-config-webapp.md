...
- **`LLMOnlyAdapter`** — generic `BackendClient` replacement, reads prompts from `pipeline_params` like any backend. Gated behind `LOCAL_EVAL_SECRET` + `local_eval_token` for multi-tenant safety. See [`security-foundations.md`](security-foundations.md).
...
| Local LLM eval = mini-connector | Technical debt, future refactor for M11 | Interface compatible with BackendClient (duck typing). Auth-gated via `LOCAL_EVAL_SECRET`. See [`security-foundations.md`](security-foundations.md) |
