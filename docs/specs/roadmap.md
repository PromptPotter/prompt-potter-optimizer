...
- **M11 — publication benchmarks + ablation + connector smoke.** BBEH primary, HotPotQA pending saturation probe, ablation rows (L1/L1+L2/full, scan, SearchMemory, critique, zero-signal filter). PromptPotter-as-connector smoke run. Webapp read-only — shipped. → [`m11-publication-benchmarks.md`](m11-publication-benchmarks.md), [`m11-spend-tracking.md`](m11-spend-tracking.md)
- **M12 — multi-connector + competitor comparison + L4 closure + composite fitness.** Second connector validates the boundary; cited competitor numbers; outer-loop run on `datasets/promptpotter/`; per-candidate cost/latency rollup → multi-objective formula. → [`m12-multi-connector.md`](m12-multi-connector.md), [`m12-control-plane.md`](m12-control-plane.md)
- **M13 — chat-first multi-user web.** One admin self-hosts; casual users sign in; chat is the constant control surface. Spec only. → [`m13-chat-first-user-web.md`](m13-chat-first-user-web.md)
- **State-sync cleanup** — collapse the five drifting state surfaces before whitelabel. → [`state-sync-cleanup.md`](state-sync-cleanup.md)
