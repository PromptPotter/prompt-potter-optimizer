...
| C8 | **Mask abstraction** — backend organizing structure (alternative-criterion + transferability); M1 = scoring-function-swap divergence + minimal visual clues, then migrate every divergence trigger onto it | spec-only (see § Lineage mask) |
...
### Lineage mask — spec-only (Milestone 1 not started)
...
- **The shape:** one shared tree-recursive fold `find_divergences(record, verdict) -> {divergences, divergent}` (a read-time `application/` service the API calls, [R-14]); each mask is a **verdict strategy** in its home; `MaskSpec` is a thin API-edge selector (function + strategy, not an internal switch, not a class hierarchy). No persistence (criterion is a request param). "Mask" is internal naming.
- **Milestone 1 (foundation):** the **scoring verdict** — "matched-sample leader under the swapped scorer (`value_with_mask_applied`, reusing `_compute_accuracy`/`materialize_round_values`/`compile_round_scorer`) ≠ `is_winner`?"; divergence = first such round, **tree-recursive**; **self-consistency** (realizing verdict reproduces `is_winner` incl. eligibility). Served Display openapi-first; frontend = marker + dimmed divergent subtree.
- **First migration — abort mask (second consumer, proves the fold):** a different verdict (log-read over `elimination_context`) on the *same* fold — "would variant V ∈ {none/A/B/combo} of the two PoBB contributors (ε-elimination `pobb_should_stop` + lock-in `_leader_locked`) abort differently here?"; invariant up to the first firing, no re-run. The real none/A/B/combo *runs* (past divergence) are a separate sibling-cycle thing.
