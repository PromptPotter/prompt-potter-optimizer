...
### Lineage mask
Owned by [`mask-projection.md`](mask-projection.md); code SoT `application/mask/`. Still open here as **Lane C8**: the write-side fork-from-divergence, where persisted mask identity finally earns its keep.
