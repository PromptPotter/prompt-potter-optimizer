...
| **L3 fork authority → AlphaZero-shaped MCTS** | Three additions that, taken together, close the structural gaps between today's *best-arm-identification over a one-armed tree* and AlphaZero-shaped MCTS: **(1) selection** — L3 gains the authority to *fork from an arbitrary ancestor* in the lineage tree when it judges the current subtree exhausted; **(2) backpropagation** — round outcomes are persisted as node statistics along the lineage so ancestor nodes carry rolling descendant-success summaries; **(3) selection rule** — L3 picks the next ancestor via a UCB-style rule (closer in spirit to AlphaZero's PUCT than AlphaGo's vanilla UCT, since our "rollout" is one full round of LLM calls — every selection has to be sample-efficient). Categorical capability unlocked: **recovery from dead-end branches**, a sequence the loop today can only stall on. Algorithmic comparison: [`docs/research/related-work.md#comparison-to-mcts`](../research/related-work.md#comparison-to-mcts). |
