...
| **L3 fork authority → AlphaZero-shaped MCTS** *(partial ship)* | **(1) selection ✓ shipped — observation-only**: L3 emits `fork_proposal: {round_offset, reason}` alongside its `plan` rewrite; lands in `round_NNNN.json::nodes[l3_plan].exit.fork_proposal`; operator forks manually via `resume --from N`. Schema: `application/optimization/dispatch/schemas.py::ForkProposal`. **(2) backpropagation** *(M13+)*: persist round outcomes as node-stats along the lineage so ancestor nodes carry rolling descendant-success summaries. **(3) selection rule** *(M13+)*: L3 picks the next ancestor via a UCB-style rule (closer to AlphaZero's PUCT than AlphaGo's vanilla UCT — our "rollout" is one full round of LLM calls, so every selection has to be sample-efficient) + wire from L3's proposal into `inherit_from(parent, offset)`. Categorical capability unlocked when all three land: **recovery from dead-end branches**. Algorithmic comparison: [`docs/research/related-work.md#comparison-to-mcts`](../research/related-work.md#comparison-to-mcts). |
