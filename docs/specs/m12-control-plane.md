...
**Status:** specced. Depends on [`m10-operator-control-loop.md`](m10-operator-control-loop.md) (single-operator write surface this milestone hardens into a SaaS).
...
- **`TenantId` / `SafeName` newtypes** plumbed through every store constructor — the single coordinated diff `security-audit.md` deferred. Mirrors `CycleDir` / `RootCycleDir`. Lite path-validation already landed.
...
[`security-audit.md`](security-audit.md) (SafeName/TenantId, prompt-injection Phase 2) · [`state-sync-cleanup.md`](state-sync-cleanup.md) Phase 1 prereq · [`m13-chat-first-user-web.md`](m13-chat-first-user-web.md) (end-state product surface).
