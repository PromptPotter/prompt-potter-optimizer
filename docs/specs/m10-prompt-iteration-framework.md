...
- **Unified fork primitive.** `_mint_fork(parent, fork_from_round, payload)` replaces `_fork_at_divergence()`. `ForkTrigger` enum carries `OPERATOR_SWEEP` / `OPERATOR_REWIND` / `L2_REBASE` / `L3_REBASE` / `SCORING_DIVERGENCE`; payload is a single `ForkPayload` shape across all triggers. `OptimizerAction.rebase: RebaseAction | None` ships in M10 unwired; M11 wires emission.
