...
- **Cross-cycle leaderboard** — `python -m promptpotter sweep rank` (reads the sweep-toolkit result JSONs). Read-only stdout; row keyed on `(l1_generate_hash, rounds_to_95 asc, behavior_pass_rate desc)`. Filters to `fork_trigger=operator_sweep`.
...
| Area | Files |
|---|---|
| Optimizer prompts | `datasets/_optimizer/pipeline.json::resolved_prompts['{l1_generate,l1_critique,l2_context,l3_plan,checkin}/1']` |
...
| Per-round trace | `campaigns/{cycle_id}/.runtime/cache/rounds/round_NNNN.json` (target) + `rounds/round_NNNN.json` (optimizer state) |
