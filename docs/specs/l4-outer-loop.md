...
4. **Enriched outer fitness (§4).** The `outer_fitness` module (normalized AUC-lift, panel mean−λ·std, ledger-reconstructed per-round spend), PoBB-decisive on `elimination_p_best` over inner-campaign arms (the dead `elevate_to_decisive` was removed), wired as the P2/P3 post-aggregate formula. **Done when:** `proxy_lift_corr ≥ 0.6` over ≥4 paired branches — the C3 exit gate.
...
| Outer fitness | new `outer_fitness` module; outer decisive-compare on `metrics.py::elimination_p_best` (the dead `elevate_to_decisive` was removed), reads `index.json` + `.runtime/ledger.jsonl` |
