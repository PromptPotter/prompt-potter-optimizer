...
- **The meta prompts must be TIGHT — gpt-oss-120b returns empty content on a verbose meta-prompt** (`raw_chars: 0`, `l1_provider_empty_response`). Keep `promptpotter/assets/optimizer/sets/meta.yaml` terse; never fix this by touching the global `assets/optimizer/pipeline.yaml` node config (shared with every inner cycle).
...
- **A stale prompt file is a live parse failure, not just documentation drift.** `promptpotter/assets/optimizer/sets/meta.yaml` (the OUTER L4 prompt set, selected by `optimizer_set: "meta"`) declared `variant_name` REQUIRED. `L1Variant` is `extra="forbid"`, so the moment that field was deleted from the model, every outer variant would have failed validation. **The Pydantic model, both `answer_format`s, and `resolved_schemas` move in ONE commit or the loop stops parsing.**
...
prompt set: **`promptpotter/assets/optimizer/sets/meta.yaml`** — prompt *fields* only. There is
