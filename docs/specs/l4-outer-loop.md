...
## 1. The shared in-process execution seam
...
**`llm_only` is a node name, never a connector.** The single-node case is served by the
**TermNorm connector accepting an `llm_only` pipeline**; re-adding a no-server connector
needs a dataset that actually declares it, not a spec claim.

## 2. In-process recursion isolation — keep it depth-agnostic
