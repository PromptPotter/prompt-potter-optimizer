...
7. **The outer SNR instrument — the scientific gate that makes this whole phase falsifiable, and the trigger for when to trust the loop.** The finish-line goal — *tuning and searching a very good base configuration for the L1/L2/L3 loop, driven by the outer loop* — is only meaningful once the outer signal exceeds its own measurement noise. Below that line the outer loop is **noise-blind** and its verdicts are coin-flips: this is why `120B`-vs-`v4-flash` was unreadable, and why round-1-then-plateau makes every optimizer prompt look identical. So a **recurring power analysis** (a *gauge R&R* on the outer gradient) is a first-class deliverable, not a diagnostic afterthought:
   ...
   - **Built as a derivation, not a re-run — it costs nothing to read.** Both series were already on disk: `rank_optimizer_prompts` walks every pp-self round file and already accumulates, per optimizer-prompt state, a list of readings per cell. Repeat readings of one (state, cell) ARE the within series; the spread of `anchor_effect` across states IS the between series. `OuterSnr` (`application/optimizer_prompt_ranking.py`) pools them and serves `within_sd` / `between_sd` / `n_cells_to_verdict` on every read, **zero spend, zero new walk** — `python -m promptpotter rank-optimizer-prompts` and `GET /optimizer-prompt-ranking`. Its per-round peer `OuterVariance` (on every `outer_verdict`) splits ONE round's spread into estimation noise vs real between-cell difference, off the per-cell `theta_se` now carried end-to-end.
   - **The reading is `UNKNOWN`, and that is the finding.** No optimizer-prompt state has ever been measured twice on the same cell, so the within series is empty and the ratio has no denominator. Every "1.9× / 1.7×" figure quoted below is the *panel's own SE*, not re-run noise; the two are different quantities and one cannot stand in for the other. Still open on top of it: the appended on-disk series / control chart (the regression guard that catches an instrument getting *worse*), which needs more than one reading to exist.
   - **Two ways this instrument can lie, both now refused.** A cell that failed is not a cell that scored zero (`domain/l4/verdict.py::cell_readings`), and a replayed reading is not a second reading (`optimizer_prompt_ranking.py::_accumulate_round`) — fixing only the first is *worse* than neither, because the surviving groups are identical, so σ collapses to 0.0 and the required sample size to zero: a perfectly sharp instrument at the moment the corpus proved nothing.
   - **The unblock is a mechanism, not just a run.** "Run one arm twice" cannot be done today: `runner/inner/cycle.py::inner_campaign_id` carries no replicate dimension, so a second measurement resolves to the finished campaign and replays it for $0, and even a distinct campaign replays every call from the `shared_root` caches. The inner instrument is deterministic by construction. Until a replicate carries its own identity end to end, `replicate_survivors` is refused at L4.
...
- **The L3 rung has never fired in a banked ledger.** Recorded so nobody reads the top rung as exercised: every L3-side claim in this repo is design intent, not measurement. Deliberately NOT acted on — re-tuning patience changes what the engine decides and voids banked origins through `_identity_config`.
...
- **The panel still cannot resolve one optimizer prompt from another.** Read `n_cells_to_verdict` off `rank-optimizer-prompts` rather than re-deriving a ratio in prose — it is served, and a served number cannot drift from the disk it came from. Two things to hold onto when reading any such ratio: `se ∝ 1/√n`, so the cell count needed to resolve is far below what a linear intuition suggests and buying cells is cheaper than it looks; and the σ is the **panel's own SE, not re-run noise**, which nothing on disk has ever measured (item 7). Until it resolves, the panel is a behavioural diagnostic, not a selector, and crowning a winner between near-identical arms is reading noise. Closing it needs more cells or **candidates that differ more than they currently do** — the cheaper lever, and the untried one.
