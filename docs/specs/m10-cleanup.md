...
**AI assistant readers, start here.** §0 (lives at
`docs/architecture.md` — see Execution order step 0) is the **entry
...
- `CLAUDE.md` (root) — onboarding pointers, project conventions,
  the §6 pre-flight gate. Carries a pointer to `docs/architecture.md`
  as the authoritative architecture reference.
...
new invariant), the layer files update in the same PR (§3 collapsing
`cadence/` into `escalation/` is the canonical example: the rename
PR also touches `application/CLAUDE.md`).
...
**after the §2/§3/§4.5 renames have landed** (the names that survive
long-term). Where today's code uses different symbols, the cross-walk
is:

| §0 vocabulary (target / long-term) | Today's symbol (pre-cleanup) | Lands in |
|---|---|---|
| `INJECTIONS` registry, `_Injection`, `InjectionKind` | `SIGNALS`, `_Signal`, `SignalKind` | §2 |
| `decide_escalation(EscalationInputs)` | split across `evaluate_round` + `firing.py` + `transitions.py`; type is `SignalInputs` | §3 step 0 + §3 |
| `ResumeCheckpoint*` records | `Decision*` records | §4.5 |
| `CycleEventLog`, `DerivedView` (subscriber base) | `CycleLedger`, `ProjectionBase` | §4.5 |
| `compile_l*_field_catalogue` | `compile_l*_surface` | §4.5 |
| `InjectionBundle` (bundle of state every prompt-fill receives) | `Bundle` | §4.5 |

This page is the **target reference**. The renames in §2/§3/§4.5 are
the work that makes the page literally true in code. When a §0 claim
about a symbol that doesn't exist yet matters for an immediate
decision, jump to the row above to find today's name.
...
- **Pipeline-agnostic.** Any backend that publishes a `pipeline.json`
  ...
  `docs/developer/pipeline-json-contract.md` (per §3.5).
...
read-only views — never write campaign artifacts of their own. **Single-writer invariant on the ledger** (pinned by
...
have this invariant today** — there are 13 raw call sites; §3.7
adds the facade that brings the archive under the same single-writer
discipline. Together, the §0 single-writer pin (here) + §3.7 facade
+ §3.8 reconstructable-state invariant capture event-sourcing's
reasoning-clarity gain without the cost of replay-on-every-read; see
"Rejected approaches" for why we don't go further.
...
modulo one current gap:** `l1_critique` is unwrapped today; §1
audit + §6 question 8 close it. Once that lands, "every optimizer
LLM call" is literally true. Events serialize to local JSONL under
...
`measurements_for_config(predicate)`) — both behind the §3.7 facade
once it lands. Cache reuse (skip backend calls when a matching
content_hash already has measurements) and cross-run LLM digests are
**derived views over this archive** — same single-source-of-truth
pattern as ledger → derived views, but at cross-cycle scope.
**The archive is the project's actual database**; the per-cycle
ledger is the event log layered on top of it. A cleanup PR that
simplifies persistence must respect both: ledger ≠ archive, neither
replaces the other.
...
must not be cut by accident. Read this list **before** §1 audit.
...
- **`compile_l1_surface` / `compile_l2_surface` field catalogues**
  ...
  scaffolding for §6 pre-flight question 1. Don't drop "because
  nobody calls it from production code today."
...
- **`scripts/ppot_review.py` + `scripts/smoke_campaign.py`** —
  ...
  test harness). Audit during §1 for accumulated cruft, but don't
  delete the underlying scripts without operator confirmation.
...
MLflow sink) live in §1's audit deliverables, not in this list. §1
either promotes them into the load-bearing list above (in a
follow-up PR) or §4 drops them.
