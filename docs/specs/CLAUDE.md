...
| [m12-api-openapi.yaml](m12-api-openapi.yaml) · [m12-events-asyncapi.yaml](m12-events-asyncapi.yaml) | **CI-checked contracts** ([`tests/test_contracts.py`](../../tests/test_contracts.py)): the closed inbound command set + outbound event set. Declare schema here *before* a handler lands. |
