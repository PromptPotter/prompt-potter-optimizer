...
| [dspy-adapter.md](dspy-adapter.md) | **Phase A closed, B mostly; C not started.** The fourth distribution tier — PromptPotter running inside someone else's program, shipped as the separate `promptpotteropt` package that depends on this one. Three phases: independent work, then closing the gap *here* so the adapter stays small, then the adapter. Produces [`../developer/dspy-optimizer.md`](../developer/dspy-optimizer.md). |
