...
| [fitness-value-model.md](fitness-value-model.md) | **Representation** sibling of fitness-comparability (that decides *which statistic gates*; this decides *how every fitness number is typed + resolved*). One tagged `Fitness(value, metric, basis)` resolved once at the serving boundary — collapses 5 backend composite-or-accuracy resolvers + 7 webapp `?? accuracy` axes + 5 hand-synced evaluator short-code maps into one. Replay (Family B) stays out by design. 5 shippable slices. |
| [schema-description-axis.md](schema-description-axis.md) | **Built, unmeasured.** `Field(description=)` is LLM-facing copy no parser reads, so it is free to mutate. Resolved at the `build_l1_output_schema` seam off the per-node override channel `layout` uses — Pydantic stays the sole default, no lift, no table. Live on `promptpotter-self`; a negative `--sweep` closes the spec by reverting. |
