...
| [schema-description-axis.md](schema-description-axis.md) | **Design, not built.** `Field(description=)` is LLM-facing copy `OptSearchPoint` can't reach — it's Python source, not overlay data. Lift into `nodes.{name}.config` → the axis exists, no new machinery. Parse-failure attribution (slice 1) shipped; still gated on a `--sweep` verdict. |
