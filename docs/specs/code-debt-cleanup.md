...
- **PRIORITY (2026-07-13): every published latency/cost figure was measured under an UNKNOWN cache state, so none of them can size a run.** The caches are tenant-global and content-addressed, so a "measurement" of how long/how much a cycle takes silently includes however many calls happened to replay for free — and nothing recorded which. Two live figures are already not comparable to how the code is wired today: `docs/operations/dataset-selection-rationale.md` quotes `justlogic` at ~0.3s/sample on `:nitro` (twice — the trial table and the recon row), but a cold-cache ledger read measured 2.1s/sample under the same suffix (the recon predates the node's `output_schema`, whose `reasoning` field makes the model emit a chain of thought — so the old number is a different experiment, not a drifted one); and the L4 throughput profile (`$X/cycle`, `Ns/cycle`) was taken warm, which is why sizing this run predicted hours and delivered ~10×. **This is not cosmetic — it is the number every "can we afford this panel?" decision reads.** Action, in order: (1) re-measure per-sample latency + per-cycle cost from a **ledger read on a cold workspace** (`.runtime/ledger.jsonl`, group `TokenUsageRecord` by `kind/node`), never from a stopwatch on a run whose cache state is unknown; (2) restate the matrix figures with the wiring they were taken under, or delete them — a number that cannot be reproduced from current config is worse than none; (3) note in each that `incurred_usd`/`used_usd` divergence is now the on-disk tell for cache warmth, so a future figure can state its own cache state instead of assuming one. Related but distinct from the spend-accounting item above: that one is about the code shape, this one is about every number we have already published from it. **The trap is live, and it caught this very entry:** an early draft recorded that nitro had *inverted* the bottleneck onto the optimizer loop. That was read off a 96-call snapshot one round into one cell; the same cell at 5 rounds (230 calls) puts the backend at 2.8s × 230 ≈ 10.7 min against ≈ 6 min for the whole optimizer loop, in a 14.2-min cell — so the backend is still the larger share and the optimizer is a close second, not the new dominant term. Both the per-sample latency (2.1s → 2.8s) and the ranking moved as n grew. Read a **completed** cell, never a running one. No blocker.
...
- **Display-only recomputes do NOT breach scoring authority** — `webapp` `hit_rate` + `headline-stats.ts::fitnessTrend` fold already-served values, and `presentation/views/live/phase.py` recomputes recall@k for the terminal readout. Serving either would add wire coupling for identical behaviour. (The bare `except: pass` around the recall block WAS the real smell, and was fixed.)
...
- **L2/L3's shared `fork_proposal` + `terminate_proposal` is ONE seam with two entry points** — L3's layout already teaches it both. Don't re-file as duplication. The fact worth carrying: **`l3_plan` has fired 0 times across all 14 banked ledgers**, so every L3-side claim in this repo is design intent, not measurement ([`l4-outer-loop.md`](l4-outer-loop.md)).
...
(substring assertions, stub-forest tests — suite cap ≤200, **164 collected as of
2026-07-16**, so the cap is not the binding constraint the old "~199" implied);
