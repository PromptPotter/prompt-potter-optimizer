...
- **Three ContextVar leaks — and the count, the list and the fix as first filed were all
  wrong.** Re-verified 2026-08-15. There are **eight** ContextVars, not seven (`_PROGRAM`,
  `connectors/dspy_module.py`, was missing); **four** have no reset path (`_MODE`,
  `_INNER_SPAWN`, the optimizer prompt overrides in `dispatch/llm_call/prompts.py`,
  `_ABORT_CHECK`); and **three** restores already do it right, not two — add
  `publish_dspy_program`/`reset_dspy_program`, a try/finally rather than a CM.
  `_CYCLE_LEDGER` / `_CURRENT_ROUND` already own `Token`-returning `set_*`/`reset_*` pairs
  (`infrastructure/llm/telemetry.py`), each with one deliberate token-discarding site.

  **Two of the four must NOT be wrapped**, so the filed "one `@contextmanager` for all" would
  have introduced bugs: `_MODE` is set once per fresh inner task that then dies, and a NARROW
  scope is actively harmful — the mode has to cover finalize or the archive reads, tracing
  sink, earned-blocks fence and optimizer clamp de-hermeticize mid-measurement. `_CURRENT_ROUND`
  is a per-round running marker with no enclosing block; the first-token-only trick at
  `run_observers.py::set_round` is a deliberate outer restore.

  What is actually owed is three targeted fixes. (1) `_INNER_SPAWN`:
  `publish_inner_spawn_context` early-returns on a falsy `cycle_id`, so a caller running
  several campaigns in ONE task (`embedded_run.py`, `teleprompter.py`, `noise_floor.py`)
  keeps a stale context and `run_inner_cycle` sandboxes into the PREVIOUS campaign's dir.
  (2) the optimizer prompt overrides leak across runs in one task — the eight test sites
  hand-calling `set_optimizer_prompt_overrides(None)` as cleanup are the symptom; any fix must
  keep "an empty set is a no-op, not a clear". (3) **`_ABORT_CHECK` self-chains**, which the
  original entry never named: `_bind_run_controls` runs twice per run in the same task
  (`runner/entry.py`, once at launch and once per rebase iteration) and the second read of
  `get_abort_check()` returns the predicate this same task bound last iteration, so the chain
  grows one lambda per rebase and keeps retired fork dirs' `pause.flag` paths live in it.
  `rate_limit.py`'s "a task that ends drops its copy" is true across tasks and false across
  rebase iterations.

  **Still do not fuse them into one settings object**, and the reason is stronger than filed:
  `_ABORT_CHECK` is *composed* at the runner seam (`own_pause() or inherited()`) — that
  composition carries an outer pause into a nested L4 cycle — and `session.pause_check` holds a
  second copy of the composed lambda, so a CM restoring only the ContextVar desyncs the
  checkpoint poll from the rate-limit poll. A shared helper belongs in `shared/` (the only layer
  everyone may reach; `shared/instrument.py` already hosts the reference implementation), and
  the primitive must stay the `Token`-returning pair with a CM layered on top — three live
  restore sites cross a non-lexical boundary and cannot be expressed as a bare `with`.
