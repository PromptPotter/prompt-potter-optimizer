...
- `sp_budget_ttest` names a t-test — VALID — `application/campaign_config.py:450` still declares `sp_budget_ttest: Annotated[int, Knob(Scope.POLICY, Estimand.SELECTION)]`; `application/optimization/l1/score/overlap.py:75` still passes it as `size=`. Count is 65 occurrences across 34 files (entry cited 81; code has evolved but the misleading name and its on-disk presence in `datasets/*/campaign.yaml` are unchanged). Blocker stands.
- `SCORING_FUNCTIONS["relu"]`, `["smoothstep"]` and `["sigmoid"]` — VALID — all three remain in `promptpotter/application/scoring/formula/matchers.py:89,91,92`; no `campaign.yaml`, no test body exercises any of them (only a prose comment at `tests/test_numerics.py:994`). Neighbor `hockeystick` is exercised at `tests/test_numerics.py:177`. Blocker unchanged.
- `dashboard.json::in_flight` — VALID — `infrastructure/projections/live_dashboard/state.py:289` serves `in_flight: InFlightCall | None`; `view.py::_handle_llm_call_progress` (line 795) discards its record (`del record`) and only reschedules persist. Webapp references are `types.generated.ts:611` and `test-fixtures.ts:240` only; no active component reads the field. `max_cells_in_flight` is a different field and IS read at `webapp/components/shell/RemoteControl.tsx:195`. Blocker unchanged.
