...
- **Per-cycle `rounds[]` served from two files, rewind truncates only one** (double-ownership rd-2 #4) — `get_cycle` (`routers/campaigns/cycles.py:93 _round_summaries`) reads `index.json::rounds[]`; `/lineage` (`routers/campaigns/lineage.py:241 _rounds_from_dashboard`) reads `dashboard.json::rounds[]`. `rewind_to_round` (`campaign_store/store.py:574` → `_rebuild_round_index`) rebuilds ONLY `index.json::rounds`; `dashboard.json::rounds` is clamped later at the next `L1_GENERATE:enter` → the two disagree in the window between a rewind and the next generate. **Deliberately NOT collapsed:** repointing `get_cycle` to dashboard loses `improved`/`label` (dashboard `RoundSummary` carries neither at top level), and the honest fix (rewind rebuilds `dashboard.json::rounds` too — the "one justified addition") is net-positive. Low severity: `resume --from N` re-enters the loop immediately, so the desync window is ~ms and self-heals. Action if picked up: rewind rebuilds both surfaces from the same survivors. Blocker: agent-verified, additive; needs the `resume --from N` → get_cycle/lineage-agree test.
- **`best_round` basis: composite-argmax (winner export) vs cumulative-accuracy-argmax (index/dashboard)** (double-ownership rd-2 #2) — `cycle.py absorb_round:626`/`replay_priors:516` argmax `best_round`/`best_sp`/`best_accuracy` on cumulative **composite_fitness**; `_apply_best` (`campaign_store/store.py:104`) argmaxes index/dashboard `best_round` on **cumulative_accuracy**. Under a non-accuracy formula the exported winner prompt comes from a different round than every display's "best." **This may be correct-by-design, not a bug:** composite IS the optimizer's objective (rightly the winner-export basis), accuracy is the familiar headline — forcing them to agree would make the deployed winner STOP optimizing the configured composite. Also entangled: `best_composite_fitness` is the L2/L3 stall comparator (`firing.py:558`), so the "flip absorb_round to the accuracy key" collapse would corrupt escalation, and splitting the two trackings is additive. **Operator decision:** (a) leave as-is + document the two bases in the `_apply_best` docstring + `architecture.md §0.5`, or (b) persist cumulative composite in `round_summary` so display learns the objective basis (additive + migration). No net-negative pure collapse exists. Note: the winner-artifact provenance cluster below assumes a settled "best" basis — settle this first.
