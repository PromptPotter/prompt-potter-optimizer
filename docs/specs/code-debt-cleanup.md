...
- **`__all__`-only export hygiene** — cosmetic public-surface trim, near-zero functional value, and several entries (`ProviderSpec`, `VersionCheck`/`PreflightFn`, the `try_parse_json` re-export in `infrastructure/llm/__init__.py`) are *documented* protocol/registry surface that legitimately belongs in `__all__`. Dedicated cosmetic pass, not a bulk sweep. (Verified NOT debt, dropped from the list: `FewShotExample.explanation` — optional LLM-facing schema field that round-trips via `FewShotExample(**ex)`.)
...
**Tier 3 — webapp folds of already-served values (NOT R-36 scorer recompute; reclassified 2026-06-19).** Inspected during the Lane C8 write-side arc and found mis-bucketed: neither reimplements a scorer — both aggregate/fold values the backend already serves, so moving them server-side would ADD a wire field + coupling for identical behaviour (against "fewer lines beats more lines at equal behaviour"). Left as-is unless a concrete reason surfaces.
- `webapp/components/dashboard/samples/columns.ts:129-145` `hit_rate` cell — sums served per-dot `m.hit` booleans (`hits/n`) for the cell next to the `measurements` column, which already reads the same `meas.length`. Not a scorer reimplementation; serving it would couple `/preview` to the `/measurement-series` source (the hits live only there) for a 2-line boolean sum.
- `webapp/lib/derivations/headline-stats.ts::fitnessTrend` (was flagged at `eval/TrendChart.tsx`) — cumulative `Math.max` running-best over the already-served per-round `composite_fitness`. A pure, centralized derivation (shared by TopStrip sparkline + TrendChart), NOT a scorer; and `dash.best` is rolling-max *accuracy*, a different quantity, so it can't even be reused. Serving a `best_so_far` series would duplicate trivially-derivable data.
