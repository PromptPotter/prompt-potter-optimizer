...
- **Duplicate "verbatim task_context" predicate** — `validators/l2_behavior.py::_check_task_context_not_verbatim` ⇄ `validators/l2_output.py::_check_task_context_verbatim_repeat` both encode "did L2's proposed `task_context` carry a real per-field delta vs the prior framing." Wired to different consumers (output → live escalation `escalation/firing/executor.py`; behavior → post-hoc `review.md` conformance), so the two registries legitimately stay split — but the predicate should share one `_task_context_has_delta(proposed, prior)` primitive so "what counts as verbatim" can't drift. Also weigh collapsing the three graded l2_output checks (`_check_task_context_verbatim_repeat` / `_check_duplicate_insert` / `_check_task_context_paraphrase_repeat`) into one "framing-staleness" reason. Blocker: confirm each graded id isn't independently surfaced to L3 before collapsing; trace both consumers.
- **`application/runner/entry.py::run_optimization` god function** — ~331 lines, 18 params incl. 5 behavior booleans (`no_divergence_check`, `fork_on_divergence`, `sweep`, `diag`, + resume/halt knobs) sequencing seed-merge → limit-reconcile → origin → task-context → scoring-split → a rebase/fork `while` loop → observer rebuild → budget gate → control-hook bind → 4-arm finalizer. Extract `_prepare_run(...) -> RunConfig` (setup) + `_run_one_cycle_attempt(...)` (loop body); group the booleans into a typed `RunMode`. Seams already exist (`_apply_limit_overrides` / `_build_budget_gate` / `_read_cycle_seed` / `_finalize_run`). Blocker: behavior-preserving refactor of the master orchestrator — verify against a live cycle exercising resume + fork-on-divergence + sweep + diag (the boolean modes interleave), not a fresh round-0. Dedicated session.
