...
- **`EVIDENCE_GROUNDING_FIELDS` is the citeable-panel-name set, NOT the injection slot set** — `escalation_panel`, `parent_panel`, `sibling_yield` are citations, not dangling slots; `escalation_panel.exploration_budget` gates the `stall_exploration` escape hatch (`validators/l1_behavior.py`). A prior sweep chased this as a dead name — it isn't; don't re-hunt the slot-set-vs-citation-set conflation. (Preserved from the deleted intuitiveness-pass.md.)
