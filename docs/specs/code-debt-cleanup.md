...
- **JSON-read sweep** — ~36 hand-rolled `json.loads(path.read_text())` across ~24 files (mostly `presentation/api/routers/*`) → route through the existing `read_json` / `read_json_tolerant` (`store/base.py`); drops redundant `.exists()` guards + try/except. Mechanical line-shaving (rides an existing channel, no new concept) — fold per-file in small commits. **Adopt-in-new-code for the coupon/BYO build:** the new `grant.json` / `api_keys.json` stores MUST ride `read_json` from day one — don't add two more hand-rolled readers.
...
- Per-provider `*_RPM` / `*_TPM` (8 `Settings` fields, all default-`None`, never set) — collapse to one rate-limit map rather than delete (the wiring is real for a free-tier operator). Small design call. **⇒ PREREQUISITE for the coupon/BYO build (Lane A2):** the per-tenant `RateLimiter` is the *other* half of the `capacity>1` gate (roadmap § Run admission), and the BYO key path wires the rate bucket per resolved key — settle the one-map shape FIRST so BYO wires into one map, not eight fields.
