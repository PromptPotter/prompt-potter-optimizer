...
- **Harbor and DSPy cells cannot run** — VALID. `domain/pipeline_parsing.py:381` reads `spend_bound` as `node.get("spend_bound")`; neither `connectors/harbor.py` nor `connectors/dspy_module.py` declares one, so every node defaults to `PipelineNode.spend_bound = None`. `application/scoring/sample_measurement.py:274–275`: an LLM node with `spend_bound is None` sets `bounded = False`, so `cell_bound` returns `SendBound(usd=None)` at line 299 — unbounded. `connectors/protocol.py:121–123` confirms: "a backend serving none cannot run under a spend ceiling. Only the recursion is `True`; `dspy` runs in this process but pays through litellm, which no admission sees." Action items unimplemented.
- **Concurrent sibling cycles of one campaign** — VALID. `runner/entry.py::_build_budget_gate:132–138`: `SpendBook` seeded from `dashboard.spend_total_used_usd` / `dashboard.spend_total_tokens` — the cycle's own `LiveDashboardProjection`, not a campaign-scoped book. `usd_cap` passed per-cycle from `campaign_config.optimization.spend_budget_usd` at call site `entry.py:160`. Two forks side-by-side each seed their own empty dashboard and are each admitted against the full ceiling. Blocker (operator decision: campaign ceiling or cycle ceiling) unchanged.
- **Swapping a model means hand-editing** — VALID. `PipelineSchema._refused:794–814` reads `unsupported_params` from the model capabilities catalogue and pins the current value when a param is refused — `response_format` half confirmed closed. `presentation/cli/commands/` listing: `verify.py`, `noise_floor.py`, `reset.py`, `maintenance.py`, `evidence.py`, `seed_screen.py`, `restamp.py`, `lifecycle.py`, `probe_reasoning.py`, `new.py`, `reindex.py`, `ab.py`, `resume_command.py` — no swap verb. Blocker (swap-verb capability) unchanged.

## verified 2026-09-21

- **Adopt-in-new-code: the new `grant.json`** — VALID. Re-test run: `grep -rn "grant.json" promptpotter/` → 0 hits. Build has not started; the premise (new stores must ride `read_json_optional`/`write_json`) stands.
- **Two host-wallet mechanisms —** — VALID. Same re-test: `grant.json` absent from code. Both arms still live — `application/jobs/quota.py::admit_launch` (admission gate) and `infrastructure/store/user_store.py::User.spend_budget_usd_total` / `token_budget_total` (lifetime ceilings). Coupon build is the blocker; unchanged.
- **`domain/run_records.py::TokenUsageRecord` lacks `key_source`** — VALID. Same re-test. `domain/run_records.py:172` carries `unsettled: bool`, not `key_source`. `presentation/api/routers/auth.py:136`: `ActivityGroupBy = Literal["model", "api_key"]` still present; provider slug still faked as the key id. Blocker (coupon build adds the field) unchanged.
- **The REST API has no inbound credential** — VALID. Re-test run: `grep -rn "bearer|HTTPBearer|APIKeyHeader|x-api-key" promptpotter/presentation/api/` → 0 matches. `presentation/api/deps.py::resolve_identity:38–43` resolves either the local operator (auth-off / dev mode with no configured providers) or reads from `request.state.identity_ctx` set by OIDC session middleware (`middleware/oidc.py`). No bearer or API-key reader on the inbound path; session cookie remains the only mechanism. Blocker unchanged.
