...
2. **Draft campaign-config knobs → one `optimization_overrides` dict (consolidation).** Each OptimizationConfig knob the new-campaign draft carries (`max_rounds`, `lock_model`, and now `mechanisms`) is hand-threaded through ~6 sites: `DraftCampaign` field + `to_wire` (`application/datasets/draft_campaign.py`), `_build_default_campaign_json` (`jobs/launcher/draft_build.py`), `_EditDraftPatch` + the edit-draft handler tuple (`presentation/api/routers/commands.py`), `DraftCampaignWire` + `DraftPatch` (`webapp/lib/api/mutations.ts`), and the OpenAPI `DraftCampaign` + edit-patch schemas (`m12-api-openapi.yaml`). Every future knob repeats all six — the per-knob tax the `mechanisms` add made obvious. **Action:** collapse to a single `optimization_overrides: dict[str, Any]` on the draft, merged onto `connector.default_optimization` at commit and patched as one object (the way `origin_prompt_fields`/`mechanisms` already send the full object) — then a new knob is zero plumbing. **Blocker:** none; behavior-preserving — verify the committed `campaign.json` is byte-identical for the default case. Do as one self-contained session (touches the live `max_rounds`/`lock_model` paths).
