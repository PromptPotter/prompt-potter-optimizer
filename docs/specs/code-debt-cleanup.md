...
- **Spend accounting still declares the `tokens/usd/unpriced` triple in four shapes.** `run_records.py::TokenUsageRecord` (canonical event) → `live_dashboard/state.py::SpendBucket`/`SpendRollup` → `results.py::CycleSpend` → `types.generated.ts`. Adding one dimension is still a four-shape edit, and the asymmetry is what costs: the billed side carries `(usd, in_tok, out_tok, unpriced)` while the incurred side carries only `(usd, unpriced)`, so every new field needs a per-shape decision about which twins it gets. The same billed-vs-incurred rationale is narrated in ~6 places (proxies/run_records/state docstrings + 2 docs). **The filed fix — "make the aggregates DERIVED folds over the canonical ledger" — is REFUTED, don't re-file it:** it does not reduce the shape count (each shape still declares the triple), and a fork's spend is seeded from its *parent's* `dashboard.json` and feeds the halt probe (`runner/entry.py` `usd_cap`/`token_cap` read `spend_total_used_usd`), so folding physical ledgers would silently change when a run halts — a fork's physical ledger starts empty by design (`CycleEventLog.inherit_from` writes nothing). Any real fix here is a shape collapse, not a re-derivation. Blocker: none, but it needs a behaviour decision on fork spend inheritance first.
