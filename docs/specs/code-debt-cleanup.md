...
- `webapp WorkflowCanvas.tsx` toolbar round `<select>` (`.workflow-round-pick`, ~lines 128-149 + the `axis`/`liveRound`/`completedRounds`/`showPicker` memos + `availableRounds` import) — **duplicate round-axis selector**: `RoundTabsStrip` (same `DashboardTab`, both visible) already reads `availableRounds(dash,isLive)` and writes the same `selection.round`; the canvas comment admits it (`:48-51`). Delete the picker; canvas keeps node selection only. Drop the `.workflow-round-pick` CSS. ~25-30 lines. High confidence.
- `infrastructure/store/campaign_store/store.py:731-830` `save_diag_fork` / `save_operator_fork` / `save_sweep_fork` — 3 one-line pass-throughs to `_write_fresh_sibling(..., kind=…)`, each with exactly one caller in `optimization/resume_and_fork/fork_siblings.py::_mint_fork` (already branches on `payload.trigger`). Delete the 3 wrappers; call `_write_fresh_sibling` at the 3 sites. `save_rebase_fork` stays (real body). Grep confirms no out-of-tree caller. High confidence.
- `webapp lib/poll.tsx` local `revalCount`/`setRevalCount` (`:402,433,595`) vs the global `lib/revalidate.ts::useRevalidation()` bus — **two revalidation concepts; the dashboard poll silently uses the lesser local one** (so it does NOT re-tick on a mutation's `bumpRevalidation()`). Unify: drop `revalCount`, pass `revalidateOn: useRevalidation()`. Verify-first — the unit-switch immediate tick must still fire (the `usePoll` interval effect re-`start()`s + `runTick()`s on switch; confirm by smoke). Also fixes the `usePoll.ts:17-19` comment drift (claims callers pass `useRevalidation()`; poll.tsx doesn't). ~6 lines + 1 concept.
- `presentation/cli/commands/verify.py:125` `cmd_verify` (189 lines) — CLI command carrying application-layer logic (composite-fitness / archive-dedup / rescore, ~`:253-281`), violating `presentation/CLAUDE.md` ("no business logic in CLI commands"). Move the bootstrap→score→aggregate→record block behind an `application/` verify use-case; keep the CLI a thin arg-parse + format shell. Med-high confidence; one call site, no duplication.
- `webapp lib/poll.tsx:118` (+ the `dashboard.json` typedoc) reference a `RunWarningsBanner` that **doesn't exist** — loop warnings actually render in `RunErrorBanner`'s second block. Fix the stale name. Trivial.
