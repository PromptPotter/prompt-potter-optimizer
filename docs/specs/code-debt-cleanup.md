...
- **Ratelimit reservation leak** — `infrastructure/llm/openai_compat.py` Groq schema-repair salvage branch returns without `reservation.close()` (normal path closes; failure path raises), so on the heaviest (repaired ~2×) calls the rolling-window TPM accounting keeps the cheap `chars//4` estimate instead of the server token count → self-throttle under-counts under load. Close the reservation before the salvage return (or share one close across success paths). Spend is the headline metric. *(Surfaced by an Explore agent 2026-06-19; verify before fixing.)*
...
- **Langfuse item-id hash fold** — two byte-identical `sha256(f"{dataset}:{query}")[:16]` (`tracing/file_sink.py`, `tracing/bridge.py`); they MUST stay identical or the two sinks register mismatched IDs, so folding into one `_dataset_item_id(dataset, query)` beside `generate_observation_id` makes the invariant structural. (The two `_sigmoid` / two slug-deriver dups are 2-callers-each — skip per the ≥3-callers extraction rule unless they net the ledger down.)
...
**Operator decision:**
- `domain/results.py::CycleResult.langfuse_trace_id` — no producer (both construction sites never pass it; `bridge.py` computes it but the return is ignored). Drop the field, OR wire `end_campaign()`'s return into `CycleResult`. Blocker: is Langfuse integration being actively built?
...
- `domain/round_diagnostics.py::RoundDiagnostics.cache_share` — always `0.0`; its render block was already deleted. Serializes to `round_NNNN.json`. Blocker: `grep cache_share .promptpotter/ -r` on operator data confirms no non-zero on-disk value (default 0.0 ⇒ old files deserialize cleanly when the field is gone), then drop.
...
- `escalation/decide.py::decide_escalation(rules=...)` default kwarg — a DI seam no test exercises beyond `DEFAULT_ESCALATION_RULES`. Confirm no test passes a custom `rules=`, then collapse to an import-time constant.
