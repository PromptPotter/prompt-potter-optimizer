...
The fork-creation dialog is therefore **edit-the-searchpoint + reconcile-the-limits** in one confirm step. The `fork-cycle` payload carries the reconciled limit overrides alongside the prompt/config edits (extends gap 1's `{from_searchpoint, pipeline_overlay, origin_prompt_fields}` with the limit fields).
