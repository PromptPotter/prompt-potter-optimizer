...
L2's prompt template (`l2_context/1` in `optimizer_pipeline.json`) carries `{{plan}}`, `{{l3_to_l2_note}}`, `{{cycle_position}}`, `{{diagnostics}}`, `{{failures}}`, `{{critique}}`, `{{l1_config}}`, `{{task_context}}`, `{{l1_signal_catalogue}}`, `{{l1_rendered_prompt}}`. `LayerStrategy.build_prompt_vars` calls `DispatchHub.fill_fixed(template, bundle)` to resolve all of them in one pass. No L2-only surface object exists — L2 is just one consumer of the global `SIGNALS` registry.
