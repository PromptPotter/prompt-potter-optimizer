...
is the single ingress for every pipeline.yaml. **Two non-negotiables:**
...
[`datasets/lca-termnorm/pipeline.yaml`](../../datasets/lca-termnorm/pipeline.yaml).
A minimal one — the GSM8K single-LLM-node pipeline — at
[`datasets/gsm8k/pipeline.yaml`](../../datasets/gsm8k/pipeline.yaml):
