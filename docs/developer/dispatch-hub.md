...
- **L1_GENERATE visibility** — `L1_POSSIBLE` (`domain/l1_layout.py`, 17 names) = `{plan, task_context, rendered_prompt, pipeline_param_catalogue, prompt_block_catalogue, diagnostics, l1_wounds, critique, answer_distribution, failing_samples, mutation_memory, axis_memory, escalation_panel, origin_strengths, archive_top_runs, rare_hit_samples, sample_transcripts}` 🧩; the rest (`l3_to_l2_note`, `l1_overrides`, `l1_signal_catalogue`, `guard_breaches`, the capability directives) are L1_CRITIQUE / L2_CONTEXT / L3_PLAN-internal — L2-internal signals are excluded so L1 can't see L2's own state.
