...
`char_cap` then catches only what those missed, and says so on the ledger (`injection_budget_overrun`). Two panels used to miss by construction, because their row budget and their cap were sized apart: `mutation_memory` (rounds × candidates × value stem) and `failing_samples` (`MISS_RENDER_CAP` × the three field stems). Both now spend a byte budget at the render site — `MEMORY_RENDER_CAP` / `MISS_PANEL_CAP` are the cap *and* the budget, one number each — and both report what the budget dropped in their own voice. `mutation_memory` renders **newest round first** for the same reason: it was built oldest-first into a single fenced section, so the cut took the most recent rounds, which is precisely the record that stops round 4 re-proposing round 1.

**The fence invariant is enforced at the backstop, not trusted to each renderer.** `_truncate_to_cap` re-closes an `<UNTRUSTED_DATASET_CONTENT>` its own slice opened: the section-drop path never breaks one, but the last-resort mid-text cut can, and an unterminated fence lets sample text run loose to the end of the prompt — a prompt-injection surface rather than a lost paragraph. Renderers still fence per section so the drop path stays lossless; `diagnostics` groups its three dataset-echoing parts under one fence and leaves the numeric parts outside it, which fixes the severing at no extra fence. **A per-item bound never bounds the composition — and nothing here tries to.** The composed total is *measured*, not enforced: `prompt_chars` on every `llm_call` record, read against the node's `OPTIMIZER_PROMPT_BUDGET_CHARS` ceiling, which marks the call ⚠ when crossed. Shedding a panel there would answer a producer's failure by choosing which half the model sees. Bound it where it is written, cap it where it is rendered, and read the sum as the alarm it is.
