...
`L1_SCHEMA_COMPLIANCE` wraps `validate_overrides()` and fires when L1's `pipeline_params_override` proposes a value outside `PipelineSchema.available_models`, outside a node's `param_allowed_values`, mismatched against the declared `param_types`, or touching an operator-locked axis (`PARAM_FORBIDDEN_KEYS`, always locked). `evidence["failures"]` is `list[ValidationFailure(axis, value, allowed, reason)]`, and `reason` is this validator's subset of the vocabulary `ValidationFailure.reason` declares: `not_in_available_models`, `not_in_param_allowed_values`, `type_mismatch`, `unknown_param`, `forbidden_axis`, `hallucinated_node`.
