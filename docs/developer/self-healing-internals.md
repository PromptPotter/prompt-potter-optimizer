...
Implementation of the four LLM-to-LLM healing loops. Concept overview: [`../concepts/self-healing.md`](../concepts/self-healing.md). This page is the wiring map.

Failures attach to the **candidate that produced them** (direct fields on `OptSearchPoint`), never to the round, so a losing candidate's problem never disrupts the round winner. New mechanisms must land as one of the four loops below — no sidecars, no silent drops.
...
`DegradationCheck` (`application/optimization/elimination.py`) fires mid-evaluation. Two paths:
...
|---|---|---|
| **Validation** (`L1_SCHEMA_COMPLIANCE`) | L1 parse, before evaluation | Loop 1 |
...
| **`DegradationCheck`** | Mid-eval, after 3 queries (or first fatal) | Loop 2 |
...
`classify_result()` (`application/optimization/elimination.py`) derives **fatal codes** from the backend's neutral advisories (`llm_only:content_empty`, `*:content_filtered`, …) and raw response shape (`pipeline_data.step_tokens.{node}`: normalised `finish_reason`, `reasoning` token count). Backend = facts, optimizer = policy.
...
- New runtime measurement pointing at a candidate config region → **Loop 2**. Add a check that emits `RuntimeFailure` from `score_population`.
