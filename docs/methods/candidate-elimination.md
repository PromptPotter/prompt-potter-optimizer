...
| 5 | **`PoBBCheck` — paired margin (futility)** — `P(net ≥ ⌈improvement_threshold·budget⌉ wins vs seed) < ε` on the stratified pairing; `need > opportunities` ⇒ `binom_sf = 0` (the deterministic can't-catch-up corner, same formula) | every query | `n_min` | eliminated; **`margin_cut`** with the full `margin` block (wins/losses/net/need/opportunities/p_w/p_clear) | — | `application/optimization/pobb/checks.py::_margin_stats` |
| 6 | **`PoBBCheck` — Bayesian** — paired `P(best) < ε` | every query | `n_min` | eliminated; records `elimination_cut` decision | — | `application/optimization/pobb/checks.py` |
