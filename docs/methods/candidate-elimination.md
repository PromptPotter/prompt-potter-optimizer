...
Individuals are evaluated sequentially on **Q** in the shared round order. The first candidate runs to completion, establishing a reference. Each subsequent candidate is measured query by query; once `elimination_n_min` is reached, after every query we recompute the **paired-margin gate** and each candidate's **Posterior-of-Being-Best** probability and stop the current candidate when either falls below ε.

The **paired-margin gate** fires before the ability posterior. It asks the round's actual adoption question on the pairing itself: crowning requires `net ≥ margin` where `net = wins − losses` (win = candidate HIT where the seed missed; loss = candidate MISS where the seed hit; ties carry nothing) and `margin = ⌈improvement_threshold · budget⌉`. Kill when `binom_sf(opportunities, need, p_w) < ε`, with `opportunities` = unattempted seed-MISS (plus unclassified) samples, `need = margin − net`, and `p_w` the Laplace-smoothed win rate **on the measured seed-MISS stratum only** — so a prefix of seed-hit ties can never inflate the extrapolation, and the statistic is order-agnostic by construction. `need > opportunities` makes `binom_sf` exactly 0: the deterministic can't-catch-up corner rides the same formula (no second code path). Losses enter through banked `net` only — a one-sided bound that never kills a candidate the full two-stratum convolution would keep.
...
- **Late — low-signal.** L2/L3 escalation has narrowed the population. True gaps ≤ 0.02. The Bayesian *best*-test can't confidently abort a near-tie (P(best) ≈ 0.5). The **paired-margin gate** does the complementary job here: a candidate improbable (< ε) to net the adoption margin against the seed is cut even while P(best) sits at 0.5 — a tie can never be crowned, so confirming it on the whole panel is wasted spend. With the shared order fronting the win opportunities, a pure tie dies at the futility cut (~n=17/24 at ε=0.15 on the justlogic shape) or at deterministic exhaustion (n=19, the last seed-miss spent); a genuine near-contender that banks a win clears `need ≤ 0` and rides to cap, where the winner is picked by the θ election.
...
| 5 | **`PoBBCheck` — paired margin (futility)** — `P(net ≥ ⌈improvement_threshold·budget⌉ wins vs seed) < ε` on the stratified pairing; `need > opportunities` ⇒ `binom_sf = 0` (the deterministic can't-catch-up corner, same formula) | every query | `n_min` | eliminated; **`margin_cut`** with the full `margin` block (wins/losses/net/need/opportunities/p_w/p_clear) | — | `application/optimization/pobb/elimination/checks.py::_margin_stats` |
| 6 | **`PoBBCheck` — Bayesian** — paired `P(best) < ε` | every query | `n_min` | eliminated; records `elimination_cut` decision | — | `application/optimization/pobb/elimination/checks.py` |

**Ordering inside the query loop.** For each query: (1) prior-result cache lookup; (2) if degraded → `execute_stale_data_protocol`; (3) `on_result` fires → display renders the line; (4) iterate every enabled check in `degradation_checks`; first to return a signal ends the candidate. Mechanisms 3–6 co-exist in that final list — fatal beats rate beats margin beats Bayesian PoBB. Inside `PoBBCheck.check()` the margin gate runs first (integer pairing arithmetic, no θ fit), then the Bayesian posterior. A tied candidate a margin cut removes has `p_best ≈ 0.5`, so it records under its **own** `margin_cut` kind with its own stratified-binomial replayer (the ε-gate replayer would not re-derive it) — the shared `binom_sf` keeps live and replay bit-for-bit. The margin gate's kill payload stamps `p_best: 0.0`, which is also what bars a cut candidate from the round election (`is_leader_eligible`).
