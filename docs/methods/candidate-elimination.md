...
`pick_value = decision_information_gain + explore_weight · model_information_gain`, with `explore_weight` (default 0.05 — low, so the queue mechanism stays decision-dominated and re-measures the informative samples it knows about) the `ExplorationConfig` knob. The seed is origin in R1 and the prior round winner R2+.
