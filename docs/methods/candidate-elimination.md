...
- **Late — low-signal.** L2/L3 escalation has narrowed the population. True gaps ≤ 0.02. The Bayesian *best*-test can't confidently abort a near-tie (P(best) ≈ 0.5). The **equivalence gate** does the complementary job here: a candidate whose observed rate makes it improbable (< ε) to clear the *adoption* bar (`seed + improvement_threshold`) is cut even while P(best) sits at 0.5 — a tie can never be crowned, so confirming it on the whole panel is wasted spend. Clear losers and can-at-most-tie candidates are pruned; a genuine near-contender still rides to cap (the gate is conservative at the same ε), where the winner is picked by point-estimate accuracy.
...
- `OptimizationConfig.pobb_epsilon` (default `0.05`) — smaller = more conservative. Shared by the Bayesian *best*-test and the equivalence *futility*-test.
- `mechanisms.elimination.equivalence_elimination` (default on) — the practical-equivalence gate. Its adoption bar is `seed + improvement_threshold`, so `improvement_threshold` (what counts as "meaningfully better") *is* the aggressiveness knob: raise it to prune ties sooner, lower it to keep them longer.
...
| 6 | **`PoBBCheck` — equivalence (futility)** — at the candidate's own observed rate, `P(clear seed_hits + ⌈improvement_threshold·budget⌉) < ε` | every query | `n_min` | eliminated; **`equivalence_cut`** with `data.equivalence` | — | `application/optimization/pobb/elimination/checks.py::_equivalence_check` |
| 7 | **`PoBBCheck` — Bayesian** — paired `P(best) < ε` | every query | **4** | eliminated; records `elimination_cut` decision | — | `application/optimization/pobb/elimination/checks.py` |

**Ordering inside the query loop.** For each query: (1) prior-result cache lookup; (2) if degraded → `execute_stale_data_protocol`; (3) `on_result` fires → display renders the line; (4) iterate every enabled check in `degradation_checks`; first to return a signal ends the candidate. Mechanisms 3–7 co-exist in that final list — fatal beats rate beats dominance beats equivalence beats Bayesian PoBB. Inside `PoBBCheck.check()` dominance runs first (pure arithmetic, certain futility vs the bare seed), then the equivalence gate (probabilistic futility vs the adoption bar — same paired-hit arithmetic on the shared universe, at the candidate's observed rate instead of dominance's optimistic rate=1), then the Bayesian posterior. A tied candidate an equivalence cut removes has `p_best ≈ 0.5`, so it records under its **own** `equivalence_cut` kind with its own binomial replayer (the ε-gate replayer would not re-derive it) — the shared `binom_sf` keeps live and replay bit-for-bit.
