...
A cycle is a directory under `campaigns/`. A fork is a new cycle whose `index.json` carries `parent_cycle_id`. The parent's ledger gets a `Decision(kind=FORK_CUT)` naming the child's `cycle_id` and the cut round; the fork's ledger inherits from the parent's history up to the cut. That's it.
...
| **Operator sweep** | `optimize --sweep` with payloads under `datasets/{name}/sweep/` | `Decision.data.fork.sweep_payload` |
...
| Sweep payload (recorded fact) | parent's ledger, `Decision.data.fork.sweep_payload` | — | ✓ on parent | — |
