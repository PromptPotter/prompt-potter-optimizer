...
*Live example.* `L1Variant` (`application/optimization/dispatch/schemas.py`) generates `evidence_grounding` **before** the delta slots it justifies (`pipeline_overlay`, `prompt_fields_updates`). Emitted after, a citation can only rationalize a mutation already made — and the `evidence_grounding_present` check would police a symptom its own schema caused.

The second correction was costlier, and it was in the **required set**, not the order alone: `changes_description` was required and generated *above* the delta slots, which were optional. A model could therefore name a change, cite a panel for it, and mutate nothing — and roughly one live variant in ten did exactly that, arriving narrated-but-empty. The schema was asking for the story and treating the substance as garnish. The payload now generates before the prose that reports it, a variant that changes nothing against its parent is rejected at parse (`L1Variant._reject_empty_mutation`, over `domain/candidate_diff.py::candidate_delta`), and `variant_name` — an unbounded free-text identifier that led the object, that no engine reader consumed, and that the decoder repeatedly derailed into repetition loops — is deleted outright. The prompt's `answer_format` lists the fields in the same order; a schema that disagrees with its own prose teaches twice, contradictorily.
...
**What the optimizer may reach, and how it is stopped.** `description` is free and open by default — no campaign toggle, because a toggle on a free lever is a fallback chain wearing a flag. It is a **core, target-level** lever: any pipeline node that ships an `output_schema` gets one param per field, `output_schema_descriptions.<path>` (dotted into nested fields, through a list's items), so `l1_generate` rewriting `llm_only`'s `{reasoning, answer}` descriptions is the ordinary case, not an L4 special case. Being ordinary params, each locks like any other: the check-in's field tree locks a field and everything under it, until a descendant is unlocked again. Pydantic stays the sole default, so there is no lift table and nothing to keep in sync. The field **name** is the one contract lever the optimizer may pull, and only after an L2/L3 fork opens `optimization.schema_field_rename` (off by default); it is safe there because a rename is a presentation transform — the emitted schema advertises the new name, a `validation_alias` binds it back, and no downstream reader observes it. **Type, `enum` values, and the schema itself have no toggle at all**: `SCHEMA_OWNED_FIELDS` (`domain/pipeline_schema.py`) subtracts `output_schema` / `schema_family` / `schema_version` / `answer_field` from every emittable param surface, so the LLM cannot emit a key that does not exist. Structural, not policed per round.

**Fenced from the optimizer, not from the operator.** `never_axis` says who may SEARCH a key, never who may set it, and reading it as an edit gate is what made `answer_field` untouchable on a fork too. The SCHEMA itself is not a config row: it is a tree — names, types, enums, the prose under each field — and the "Structured output" view renders it, resolved at this searchpoint. A check-in authors one in that view, field by field; until a node has one, the toggle below stays pinned to `text`.

**Whether the node uses a schema AT ALL is a different lever, and it is OURS.** `response_format` is synthesized onto every LLM node at parse time (`SCHEMA_TOGGLE_PARAM`) rather than declared by the backend, because PromptPotter is what puts the schema on the wire. `json` sends `output_schema` + `answer_field`; `text` sends neither, and the answer is then found by the matcher's own contract (`formula/matchers.py::EXTRACTION_NOTES`, authored into the origin at check-in). Two bounds ride the value space rather than a badge, since L1 is shown the space, emits into it and is validated against it: a node declaring no schema has nothing to switch to, and a model that does not accept the key cannot be asked for a structure at all (`PipelineSchema._refused`). One value is the pin, so both draw as a shut row. **Why a connector must not declare it too** — owned by [`../developer/node-standard.md`](../developer/node-standard.md) § The shape, and what PromptPotter reads of it.
...
That asymmetry is the reason these two are the optimizable ones, and it is why a schema-mutating optimizer must be scoped to them explicitly — handed the whole schema, it will happily rename `candidate` and take the pipeline down.
