...
The zero-signal filter uses this: queries that always hit or always miss across enough observations are physically moved out of the active dataset. The exploration/exploitation sample selector (scoring-set evolution) uses it too, in a gentler form — queries whose difficulty is already well characterized are swapped out of the active scoring set in favor of queries whose difficulty is still uncertain and informative to measure.
