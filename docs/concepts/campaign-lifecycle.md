...
1. **AxisIndex refresh.** New evaluation data from the completed round is folded into the axis-side derived view. The next round's L1 Generate has an updated view.
...
3. **Exploration / exploitation rebalance** (on by default). A Rasch IRT posterior tracks per-query difficulty; between rounds, well-characterised queries are swapped out of the active scoring set in favour of high-information ones. The scoring set is held fixed within a round.
