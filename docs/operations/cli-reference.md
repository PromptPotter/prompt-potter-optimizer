...
When the positional is a **file** (`Path.is_file()`), `new` parses it into a durable **check-in campaign** (the same one the web ingest mints — it shows in the sidebar and survives a restart), runs the AI origin check-in (the same `checkin/2` node the web uses), auto-confirms high-confidence findings, then — once the deterministic readiness gate passes — **flips the check-in to `active` and runs the loop inline** (rich terminal display). It reuses the exact orchestration behind the web onboarding (`ingest_draft` → `resolve_origin_turn` → `prepare_checkin_run`); the file branch adds no parallel mint logic. The only CLI↔web difference is run-invocation: the CLI runs inline; the web `/commands/start-checkin` detaches. If the backend is down at this point the check-in is left ready (nothing lost) — finish it later with `resume` once the backend is up.
