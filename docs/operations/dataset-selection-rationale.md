...
**But** at the optimizer's current maturity, BBEH is the wrong *iteration* target for L1 optimizer prompt evolution. `gpt-oss-20b @ low` scores low enough on BBEH that every cycle ties at noise, PoBB can't separate candidates, and the L1 self-optimizing campaign can't tell good edits from bad ones. (This line used to cite our in-house BBEH-mini reading as *debunking* the public figure. It does not: that gap is an open **publication blocker**, owned by [`../research/bbeh-comparison/README.md`](../research/bbeh-comparison/README.md). Do not quote it as a result.)
...
| **JustLogic** | depth ≥ 4, 25 samples | low | **52% (13/25)** — VOID against the live cut | Label-bias coast: 11/25 gold = `Uncertain`, model defaulted to `uncertain` on 15. Always-predict-uncertain baseline = 44%. Real reasoning lift over mode-prediction ≈ 8pp. **A separate `dataset_name` from `justlogic-d234` sharing no cache key**, so reading this number against the live cut measures the keying, not the capability (§ Wired — primary). |
