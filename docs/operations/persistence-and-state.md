...
**A cut has a DIRECTION, and only the trigger knows it** (`FORK_DIRECTION`, derived — never stored, so every fork already on disk answers it). An operator's sweep / diag / steered fork is an `offshoot`: the child hangs off a line that keeps running. A `scoring_divergence`, `operator_rewind` or L2/L3 rebase **supersedes**: the child is the continuation the active pointer moves to, and the *parent* is what was left behind. Same shape on disk, opposite reading — which is why nothing is deleted on a supersede: whatever the old package produced above the cut, the stale candidate cache included, simply stays with the branch that produced it. Served on the lineage tree as `fork_direction`; the forest marks a supersede `↳`.
...
**Unless a repair lands.** Every resume first makes each closed round re-derive from its own rows — re-measuring cells it recorded without a measurement (a winner crowned on a holed panel; the panel gate only stops the round it fires in) and re-projecting a headline that no longer matches its winner's row. That is *incompleteness*, not divergence, so it runs whatever the config diff says. The repair is then priced by what it REACHED: every round's optimizer packages are fingerprinted before and after, each rendered at its own point in the run. **The repair itself writes nothing** — where a correction belongs is not knowable until it is known what it reached. A repair no panel rendered rejoins the line it came from (which is to say it never branched) and is written in place. One that MOVED a package is a CUT: the branch is taken first, so the parent stays byte-identical to what ran — its pre-repair round and its stale candidate cache both still inspectable — and the corrected rounds, with the re-distilled critique, are written onto the fork. Persisting first is what left a parent holding a repaired round beside descendants generated from the version it replaced. The winner replay above still runs on top, so a flipped crown forks rather than overwrites.
