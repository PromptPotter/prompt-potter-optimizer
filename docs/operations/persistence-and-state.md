...
Material facts land on disk in human-readable form; reads happen by opening files (`evidence` is the one read verb — a comparison across subjects is in no single file). Entry points never write campaign artifacts directly — every write rides the per-cycle ledger through two projections (live telemetry + audit). The allowlist is a structural invariant that fails loud (an out-of-allowlist write shows up in the file tree); no standing test, see [`../../tests/CLAUDE.md`](../../tests/CLAUDE.md).
...
**One verb, two acts, and `keep_rounds` is which.** Unset it is the steer above: a clean offshoot from the origin, rounds numbered from 1, re-scoring the edited searchpoint. Set, the same command mints an `operator_rewind` instead — rounds `0..N-1` are LIFTED and the branch continues at N under the seed's `config_overrides`. That is the shape an applied scoring mask takes, because the round the preview says the two readings part at is the round everything before it still stands (`mask-projection.md` § Applying one). It refuses a seed carrying `origin_prompt_fields`: the lifted round 0 already IS the origin, so declaring a second one asks for two at once. The terminal's equivalent gesture is `resume --rewind N`.
...
**One press buys ONE ROUND of scoring, on every backend** — the round that scored under the
depth clears it, which is the only control loop such a walk ever sits inside. The backend
declares the CEILING and nothing else (`Connector.max_cells_in_flight`); it cannot declare how
long a press lasts, because it cannot see the round. It is ONE served number, the depth in
force, re-read by the walk at every launch. **Either layer is set by naming the one you mean**,
so an outer press holds several inner campaigns in flight and an inner one several of its own
rows. The ceilings, and why a depth is never inherited downward:
[`../specs/api-openapi.yaml`](../specs/api-openapi.yaml)`::setSampleLookahead`.
