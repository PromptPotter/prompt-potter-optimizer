...
### Short codes — what `acc`, `H`, `lat`, `R`, `pc` stand for

The five abbreviations in `composite_formula_short` map to these inputs:
...
`dashboard.json::composite_formula_short` re-inlines the resolved values into the formula after every candidate scores, so a live tail reads the formula and its inputs in one line:

```
0.65*acc|0.667 + 0.15*H|0.972 + 0.10*lat|0.965 + 0.05*R|0.450 + 0.05*pc|0.812
```

For per-candidate values, the underlying registry numbers live on `current_round.nodes.l1_score.output.candidates[].stats.evaluators` (`accuracy`, `error_rate`, `latency_norm`, `prompt_compactness`, …).
