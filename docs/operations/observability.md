...
Every observability event is appended to `campaigns/{cycle_id}/events.jsonl`. This is a pure mirror — nothing reads it for state reconstruction; it's there for debugging and post-hoc inspection. Each line is a JSON object with phase, event type, round, and payload.
...
| L1 Critique | LLM call | critique-phase inbox, structured output |
| L2 Refine | LLM call | refine inbox, new directive / task_context |
| L3 Plan | LLM call | plan inbox, new plan text |
