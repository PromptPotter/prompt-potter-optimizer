...
| `justlogic-d234` | `openai/gpt-oss-20b:nitro` (OpenRouter) | `low` | absent | JustLogic (Chen 2025), 3-class deductive reasoning (`TRUE`/`FALSE`/`Uncertain`). The live L4 inner instrument: an iid random mix of depths 2, 3, 4 (200/depth from HF `train`, seed=42, interleaved). Synthetic generation — no contamination. L1 attack surface: hedge-break (the model over-predicts `Uncertain` when the derivation is unfinished). `:nitro` routing for speed. Prior `justlogic` d6-7 origin/band numbers are VOID (data-deprecation-era) — see `datasets/justlogic-d234/dataset.md`. |
