...
python -m promptpotter new <file.csv> --set task_description=…  # fresh from a RAW file: ingest → resolve origin check-in → commit tenant dataset → run
...
`new` and `resume` are the two write verbs. `new` takes a dataset **name** *or* a raw **file** (CSV): a name uses an authored `datasets/<name>/`; a file is ingested → its origin resolved by the AI check-in → committed as a tenant dataset (`projects/{tenant}/datasets/{slug}/`) → run, the headless twin of the web onboarding (dispatch is `Path(arg).is_file()`; the file branch then falls through to the same mint+loop). Dataset config resolves tenant-first via `resolve_dataset_config_dir` (carried on `Session.dataset_config_dir`), so an ingested slug is first-class to both `new <slug>` and `resume`. `new <name>` mints a fresh Campaign every call — `campaign_id = {dataset}__{rand6_hex}`, distinct per invocation — plus its root cycle, running from round 0. The declaration (target hash + optimizer-prompt hash) is recorded as properties on `campaign.json` for drift detection on resume, not as the id. Two `new` calls on an unchanged declaration share their root cycle id (`cycle_<target_hash>` is content-addressed) and origin score (the dataset-scoped archive cache-hits every sample), but get independent `campaign_id`s and diverge from round 1 onward. `resume` (bare) reads `active_session.json::{campaign_id, cycle_id}` and picks up that cycle.
...
- **Three-layer loop + recursive L4.** L1 generates every round; L2 fires on L1 stall (refines `task_context`, never `pipeline_params`); L3 fires on L2 stall (replans). **L4 is recursion, not a 4th layer** — the same PromptPotter applied to itself via the `promptpotter` connector (`promptpotter/connectors/promptpotter.py`), mutating an inner cycle's meta-prompt templates as `pipeline_params`. Conceptually L2 / L3 / L4 are the **same family** — each mutates a slower-changing surface of the level below (L2 → `task_context`; L3 → `plan`; L4 → meta-prompt templates) — but structurally L4 is another cycle, NOT a new `LayerStrategy` driver inside `application/optimization/`. There is no `l4_*.py` and there will not be one. The four loop calls go through `build_bundle → DispatchHub.fill_l1|fill_fixed → compile_prompt → LLM`; a fifth optimizer node, `checkin` (renamed from `restructure`) — one node, two modes: task-decomposition at `new` + origin-resolution at ingest — runs *outside* the loop via `run_optimizer_node`, no bundle. **No prompt site summarizes its own data.** Agent contracts: [`promptpotter/application/optimization/CLAUDE.md`](promptpotter/application/optimization/CLAUDE.md). L4 spec: [`docs/specs/m12-multi-connector.md#track-15--promptpotter-as-connector`](docs/specs/m12-multi-connector.md); concept: [`docs/concepts/optimizer-of-the-optimizer.md`](docs/concepts/optimizer-of-the-optimizer.md); dataset: [`datasets/promptpotter-self/`](datasets/promptpotter-self/). Signal routing + healing + four wound channels: [`docs/developer/dispatch-hub.md`](docs/developer/dispatch-hub.md).
- **Five I/O kinds (invariant).** **Persistence** — sole ingress `CycleEventLog.append`; `RunCallbacks` is the writer API; ledger-subscribed projections own everything else. Per-cycle ledgers are canonical; workspace-scoped commands without a cycle target (`register-backend`, `sync-backend-experiments`) ride a sibling workspace ledger at `projects/{tenant}/.workspace/events.jsonl`. **Display** — ledger subscribers; allowlist-enforced. **Control-local** — `stop_check` on `Session` + sanctioned `.runtime/stop.flag`. **Control-remote** — HTTP commands ride the canonical ledger as `CommandRecord` via sole `CommandDispatcher` (kwargs-only `emit_command`); the runner acks via `CommandAckRecord` (sole `RunnerCommandSubscriber`); outbound `ProjectionEnvelope` frames fan out over SSE via sole `EventStreamView`. Closed inbound set: `docs/specs/m12-api-openapi.yaml`. Closed outbound set: `docs/specs/m12-events-asyncapi.yaml`. Permanent contract: [`docs/adr/0001-m12-control-plane.md`](docs/adr/0001-m12-control-plane.md). **Identity** — OIDC verification at the API trust boundary; `presentation/api/middleware/oidc.py` (Stage 1) populates `IdentityContext` from a verified ID Token. Tokens never appear past the middleware (ADR-0002 gate #2, CI-checked); Stage 0 (auth-off) uses `default_identity()`. Permanent contract: [`docs/adr/0002-identity-foundation.md`](docs/adr/0002-identity-foundation.md). Forks ride Persistence via `inherit_from(parent, offset)`. **Entry points MUST NOT write campaign artifacts directly.** Layer contract: [`promptpotter/infrastructure/CLAUDE.md`](promptpotter/infrastructure/CLAUDE.md).
...
- **Identity seam (Stage 0 shipped).** `IdentityContext` (`shared/identity.py`) is the sole carrier past the resolver; `TenantId`/`UserId`/`Issuer`/`SafeName` newtypes live in `domain/identity.py`. `build_stores(identity, *, projects_root=…, datasets_root=…)` is the only `Stores` construction route; `Session.identity` replaces the deleted `TenantContext`; FastAPI seam is `resolve_identity` → `IdentityDep` → `StoreDep` (`presentation/api/deps.py`). Stage 1 (M12 OIDC client) only flips `resolve_identity` — no other call site changes. Full spec: [`docs/adr/0002-identity-foundation.md`](docs/adr/0002-identity-foundation.md), first consumer [`docs/adr/0003-spend-and-tenancy.md`](docs/adr/0003-spend-and-tenancy.md). The Stage-1 Identity I/O kind amends §0 first per Pre-flight gate Q4 sub-rule below.
- **Token/cost on the canonical ledger.** Every LLM call — optimizer (`dispatch/llm_call/call.py`) and backend (`scoring/sample_measurement.py`, per-step over `step_tokens` on uncached samples) — emits one `TokenUsageRecord` through `emit_token_usage(**kwargs)` (`infrastructure/llm/models.py`), which reads the active cycle ledger + round from the `_CYCLE_LEDGER`/`_CURRENT_ROUND` ContextVars and appends to the same `events.jsonl` that carries `PhaseRecord`/`SnapshotRecord`/`LLMCallRecord`. No process global, no wrapper dataclass, no parallel pipeline — tokens ride the canonical Persistence stream alongside every other record. `LiveDashboardView._handle_token_usage` is the **sole writer** of `dashboard.json::spend` (backend + loop buckets routed by `record.kind`); the halt loop reads `LiveDashboardView.spend_total_used_usd` (clean accessor, no probe into `state['spend']`). `AuditTrailView` continues to record the same `TokenUsageRecord`s into `round_NNNN.json` for the audit trail. New per-call telemetry follows the same shape: define a `*Record` in `domain/run_records.py`, add an `emit_*` helper that reads the ContextVar and appends, register the projection subscriber. Full spec: [`docs/adr/0003-spend-and-tenancy.md`](docs/adr/0003-spend-and-tenancy.md).
...
4. **Is the name self-describing without opening another file?** Read
   ...
   - **Sub-rule: new I/O kind?** §0 names five (Persistence, Display,
     Control-local, Control-remote, Identity). A genuinely new kind is
     an architecture-spec change — **amend §0 first**, then write the
     code. New Control-remote command or event kinds also need their
     schema declared in `docs/specs/m12-api-openapi.yaml` /
