...
| `dispatch_hub.INJECTIONS` typed dict | `application/optimization/dispatch/hub.py` | Each entry is a `_Injection(name, kind, render, doc)`; `validate_template()` (called from `load_optimizer_prompt`) raises on `{{slot}}` names not in the registry. A typo in a template fails at module load, not at first render |
