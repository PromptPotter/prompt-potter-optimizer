...
- ⚠️ **Renaming a `CampaignConfig` / `Campaign` field is a data migration, not a code change.** Both are `extra="forbid"` and `campaign.json::config` embeds a config snapshot, so a rename makes `load_campaign_config` raise `extra_forbidden` on every campaign already on disk — `resume`, `ab`, `verify`, `noise-floor` and L4's inner cycles all die at load, before any scoring. It has fired twice: `5c0722a1` (50 of 177 campaigns) and an `optimization.exploration` flatten (156 of 169), each found by hand, days late. Pre-release that costs a re-stamp of our own disk; **a distributable `promptpotter-self` cannot re-stamp a paying user's campaigns** — those are measurements we don't own. "No backward compatibility" licenses breaking *code*, never a user's data.
  ...
  **Two corrections to what this note used to claim**, both verified against real on-disk data: (1) the load-bearing surface is exactly **one** file, `campaign.json::config` — the ledger's copy is serialized but never deserialized, `draft.json` carries the draft's own UI vocabulary, archived diagnostic runs hash *pipeline_params*, and `session.json::campaign_config` is deleted. (2) **The delta is not rename-proofing** — measured on the 156 broken campaigns it would have spared 41. The fixture is the guard; the delta only shrinks the blast radius.
