...
⚠️ **A round is won on θ, not accuracy — when the operator reads the accuracy column, say so.** Subsets move between rounds and PoBB truncates arms at different depths, so a lower-accuracy winner is normally *correct*, not a bug: push back rather than accept the premise. Five states where θ is NOT ability and the pushback is wrong, all SERVED as a `ThetaCaveat` so no surface re-decides one — an arm at 0.0 on every cell (pins to a floor constant, every lift reads `0.000`; per-CANDIDATE, so it rides the candidate row while the other four ride the round's reading), a cold ruler (θ is just logit-accuracy on its own subset), a flat ruler (the instrument, not the draw — no round could have read wider), **a collapsed selection band** (the acquisition buys one narrow δ range, so θ reduces to logit-accuracy + a constant while the ruler stays warm and every number renders), and **unmeasured difficulty** (most of the round's cells share a δ the ruler gave several at once — that is the prior, not a reading, and it MOVES as the ruler grows, so an unchanged prompt drifts upward round on round). The last two are the SILENT pair; against both, read the LIFT rather than the level. **Read the round's `overlap` line before arguing either way** — C0 and every winner since, on one shared set of cells, needing no ruler. Owned by [`docs/methods/verdict-resolution.md`](docs/methods/verdict-resolution.md) § Reading a round.
