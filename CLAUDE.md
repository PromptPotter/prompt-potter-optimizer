...
3. **Optimize** — feedback cycle (LLM candidate generation → backend evaluation → winner selection) runs iteratively until the human stops it. When scan data is available, operates in **scan-aware mode**: `prepare_scan_context()` formats scan analytics into the LLM meta-prompt, enabling per-candidate `pipeline_params_override` so each candidate can explore different pipeline_param combinations informed by sensitivity data.
