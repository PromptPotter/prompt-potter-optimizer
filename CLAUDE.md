...
- **events.jsonl is the write-ahead log** — `obs/langfuse/events.jsonl` is the single source of truth for campaign state; every durable mid-round change lands as an appended event carrying a full `OptSearchPoint` snapshot. `optimize --from <cycle_id>:<event_ref>` is a pointer into that log. See [`docs/architecture/optimization.md § Forking a campaign`](docs/architecture/optimization.md#forking-a-campaign).
