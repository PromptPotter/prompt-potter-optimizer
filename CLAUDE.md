...
**`application/recon/` is DORMANT — a museum exhibit, not a backlog.** The sensitivity-scan / recon pass is preserved as a working-shape reference for possible future revival; it has zero callers in the active loop (no CLI subcommand, no UI wrapper, no L1 parameter, no `CampaignConfig` field references it). **Treat it as read-only.** Do not refactor, "improve," modernize, fix, or update it. Do not remove it. If a test breaks under recon, skip it (`@pytest.mark.skip(reason="recon is dormant")`) — do not fix it. If a lint or type rule fails, exclude the file — do not change the code. The directory's preservation is the point. See [`promptpotter/application/recon/README.md`](promptpotter/application/recon/README.md) for the same statement at the source.
