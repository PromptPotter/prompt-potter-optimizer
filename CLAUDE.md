...
`new` and `resume` are the two write verbs. `new <name>` find-or-creates the Campaign for the dataset's declaration (`campaign_id = {dataset}__{declaration_hash}` — dataset + target + optimizer meta-prompts; stable across re-runs) and mints a fresh Session + its root cycle inside it, running from round 0. `resume` (bare) reads `active_session.json::{campaign_id, cycle_id}` and picks up that cycle, extending the active session.
...
- **Pipeline params + scoring.** `pipeline_params` always nested dicts keyed by node — no flat format, no `override_map`. `PipelineSchema` built entirely from `GET /pipeline` — zero backend constants. `score_search_point()` is the single scoring gateway. Each trace carries `{scorer_id: {score, hit, formula}}`; resume halts on first decision divergence; `--fork-on-divergence` mints a sibling rooted there. Hot-swap composite via `campaigns/{campaign_id}/cycles/{cycle_id}/scoring_steer.json`.
- **Cycle + campaign identity, scoring-set mutations.** Cycle id = `cycle_{target_hash[:12]}` from origin `JobSearchPoint.content_hash(dataset)` (rendered target prompt + dataset + target `pipeline_params`). Campaign id = `{dataset}__{declaration_hash}` — `declaration_hash` folds the target hash with the **optimizer** meta-prompt hash (`datasets/_optimizer/`), so editing an optimizer prompt mints a distinct campaign. `campaign.json` records `root_content_hash` (target) + `optimizer_prompt_hash`; resume recomputes both to detect drift. Path resolution is `(campaign_id, cycle_id)`. The scoring bank is the full train split; the zero-signal filter (off by default) is the sole round-boundary writer that mutates it (`datasets/{name}.json::excluded`). The per-round eval subset is a round-local read-and-select — the CAT picker (`select_round_subset`) narrows the bank to `sp_budget_ttest` samples each round — not a writer of the bank.

## Persistence

Four entities, outermost → innermost: **Workspace** (`projects/{tenant}/` — tenant datastore) → **Dataset** (`datasets/{name}/`) → **Campaign** (first-class — `campaigns/{campaign_id}/`, `campaign_id = {dataset}__{declaration_hash}`, stable: re-running `new` on an unchanged declaration — dataset + target + optimizer meta-prompts — find-or-creates the same campaign) → **Cycle** (root | fork | diag | sweep, `cycle_{hash[:12]}`). A campaign is a forest of N **sessions** — one per `new` run on its declaration; `resume` extends the active session, it does not add one. The session id is the existing `session_id`. `.promptpotter/` holds two trees: `sessions/{session_id}/` (operator workspace) and `campaigns/{campaign_id}/`. The campaign dir holds `campaign.json` (manifest), `log.md` (digest covering every session + forks + rounds), `hard_samples.json` — with every cycle (all N session roots plus every fork/diag/sweep) flat under `cycles/{cycle_id}/`; sibling kind + sweep batch id are `index.json` metadata, never directory nesting. `dashboard.json` is per-session — it lives in the session's root cycle dir (`cycles/{session_root}/dashboard.json`), so a campaign has N live streams. Per-cycle audit (`index.json`, `log.md`, `rounds/`, `langfuse/`, `prompts/`) at each cycle's top level; runtime internals (ledger, cache, P(best) streams) under `.runtime/`. `archive/measurements/` is the MeasurementArchive — DB core, peer of `campaigns/`, queryable at three scopes (campaign / dataset / workspace). **Reads happen by opening files** — no read CLI. Full tree, fork lineage, and recovery workflows: [`docs/operations/persistence-and-state.md`](docs/operations/persistence-and-state.md). Layer contracts: [`promptpotter/infrastructure/CLAUDE.md`](promptpotter/infrastructure/CLAUDE.md).
