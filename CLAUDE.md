...
**Adaptive sample prefix — Rasch + Knowledge Gradient.** Off by default (`CampaignConfig.optimization.adaptive_prefix.enabled`). When on, `runner.py::_maybe_evolve_adaptive_prefix` runs at each round boundary: refits a Rasch IRT model `P(hit) = σ(θ_c − δ_s)` on the accumulated `(candidate, sample, hit)` triples, then swaps prefix samples with narrow CI on `δ_s` for non-prefix samples with high Knowledge Gradient. The prefix stays shared *within* a round (Wilcoxon pairing intact) and only changes *between* rounds — Rasch handles the resulting sparse observation matrix natively. All swap decisions reduce to float thresholds (`swap_out_delta_se`, `swap_in_kg_threshold`, `max_swaps_per_round`); the floor is `min_prefix_size` ≥ `elimination_n_min`. Hardness leaderboard falls out for free as a derived view over `δ_s`. Per-round events persist on `Cycle.prefix_events` → trial JSON; rendered by `presentation/views/adaptive_prefix.py`. Full mechanics in [`docs/methods/adaptive-sampling.md`](docs/methods/adaptive-sampling.md).
...
- **Round-boundary scoring-set mutations** — two sanctioned writers, in this order: (1) **Zero-signal filter** (off by default, `min_observations=5`): queries with 0 variance across ≥ N samples physically moved to `datasets/{name}.json::excluded`; mutates the on-disk dataset. (2) **Adaptive sample prefix** (off by default): Rasch + KG swaps low-info ↔ high-info samples in the in-memory `session.scoring_dataset` only; never touches disk. No other mutation of either is sanctioned.
...
**Concepts (how it works, concept-first)**: [`campaign-lifecycle.md`](docs/concepts/campaign-lifecycle.md), [`three-layer-loop.md`](docs/concepts/three-layer-loop.md), [`self-healing.md`](docs/concepts/self-healing.md), [`scoring-and-traces.md`](docs/concepts/scoring-and-traces.md), [`search-memory.md`](docs/concepts/search-memory.md), [`prompts-and-candidates.md`](docs/concepts/prompts-and-candidates.md), [`nodes-and-pipelines.md`](docs/concepts/nodes-and-pipelines.md), [`glossary.md`](docs/concepts/glossary.md)
...
**Methods**: [`candidate-elimination.md`](docs/methods/candidate-elimination.md), [`adaptive-sampling.md`](docs/methods/adaptive-sampling.md)
