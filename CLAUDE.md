...
- ⚠️ **Renaming a `CampaignConfig` / `Campaign` field is a data migration, not a code change.** Both are `extra="forbid"` and `campaign.json::config` embeds a config snapshot, so a rename makes `load_campaign_config` raise `extra_forbidden` on every campaign already on disk — `resume`, `ab`, `verify`, `noise-floor` and L4's inner cycles all die at load, before any scoring. It has fired twice: `5c0722a1` (50 of 177 campaigns) and an `optimization.exploration` flatten (156 of 169), each found by hand, days late. Pre-release that costs a re-stamp of our own disk; **a distributable `promptpotter-self` cannot re-stamp a paying user's campaigns** — those are measurements we don't own. "No backward compatibility" licenses breaking *code*, never a user's data.
  **What is in place now.** The snapshot persists the **delta from defaults** (`freeze_campaign_config`), so a knob nobody set is absent and renaming it is free — but a knob the operator *did* set is written down and still breaks. That residue is caught in CI by `tests/fixtures/cycles/frozen_campaign/` (a pinned `campaign.json` through the real store reader), and remedied by `python scripts/restamp_campaign_configs.py --apply`. **Ship a rename together with a re-stamp**, never `extra="allow"`, an alias, or a migration shim.
  **Two corrections to what this note used to claim**, both verified against real on-disk data: (1) the load-bearing surface is exactly **one** file, `campaign.json::config` — the ledger's copy is serialized but never deserialized, `draft.json` carries the draft's own UI vocabulary (`lock_model` → `forbidden_axes_strict` at commit), archived diagnostic runs hash *pipeline_params*, and `session.json::campaign_config` is deleted. (2) **The delta is not rename-proofing** — measured on the 156 broken campaigns it would have spared 41. The fixture is the guard; the delta only shrinks the blast radius.
...
- **Roadmap:** three ways to run it — **(1)** hosted beta (`app.promptpotter.dev`), **(2)** local `/potter-run`, **(3)** the self-hosted **team-online** stack ([`deploy-linux/`](deploy-linux/); Cloudflare tunnel + OIDC allowlist + quotas), **which is what the developers/operator run** (tiers 1 & 3 share this stack). Engine + webapp + control plane + chat (Arc 1) + ingest shipped; Lane A (BYO keys) + Lane C (chat write-path, L4, composite fitness) in flight → [`docs/specs/roadmap.md`](docs/specs/roadmap.md).
...
- **Skills:** [`.claude/skills/`](.claude/skills/) — `potter-run` (launch + supervise a campaign), `potter-improve` (self-improve the meta-prompts), `potter-l1-meta-campaign`, `potter-debt-sweep` (the daily hygiene sweep), `design`.
