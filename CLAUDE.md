...
- **Skills:** [`.claude/skills/`](.claude/skills/) — `potter-run` (launch + supervise a campaign), `potter-debt-sweep` (the daily hygiene sweep), `design`. Self-improving the meta-prompts is L4's job (`new promptpotter-self`), not a skill's: the hand-driven sweep loop it replaced is gone.
