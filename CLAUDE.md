...
- 🔴 **URGENT (fix by 2026-07-15) — a `CampaignConfig` field rename bricks every campaign already on disk, silently, and nothing catches it.** `campaign.json` embeds the *whole* config snapshot (every leaf, defaults included) and `CampaignConfig` is `extra="forbid"`, so renaming one field makes `load_campaign_config` raise `extra_forbidden` on every pre-existing campaign. `resume`, `ab`, `verify`, `noise-floor`, and **L4's own inner cycles** (`runner/inner_recursion.py`) all die at load, before any scoring. **This already happened:** `5c0722a1` folded `deterministic_dominance` + `equivalence_elimination` → `margin_elimination` and broke 50 of 177 local campaigns; it went unnoticed until `ab` was run days later, and needed a 941-file re-stamp (a snapshot also rides `session.json`, `ledger.jsonl`, `draft.json`, and the archive runs — four more file kinds).
  **Why it must be fixed now, not at leisure:** pre-release we can re-stamp our own disk. **A distributable `promptpotter-self` cannot** — a paying user's campaigns are data we don't own and can't rewrite, so the next rename is a silent upgrade-time data loss on their machine. "No backward compatibility" licenses breaking *code*, never a user's measurements.
  **Root, not symptom:** two causes, both upstream of the crash. (1) The snapshot persists **every leaf** (40 of them) rather than the delta from defaults, so a rename of a field nobody ever set still breaks the file. (2) **No test feeds a previously-written `campaign.json` through `load_campaign_config`** — `tests/test_resume.py:390` passes a freshly-built dict, which by construction can never carry a stale key. This is a *silent* harm (a `resume` that cannot load is lost measurement data), so it is in charter, not excluded by "breaks loud". Fix: persist the config **delta**, so a renamed default-valued field simply vanishes; and pin a frozen `campaign.json` through `load_campaign_config` via the existing [`cycle-fixtures`](docs/developer/cycle-fixtures.md) mechanism (`tests/fixtures/cycles/`) — no new infrastructure. Do **not** reach for `extra="allow"`, an alias, or a migration shim; those are the forbidden shapes. Where a rename is still unavoidable, ship it with a re-stamp verb, never a shim.
