...
- **Persistence** (shared, mandatory) — two newtype-guarded projections under `infrastructure/projections/`: `LiveDashboardProjection` (takes `RootCycleDir`, owns `dashboard.json` + `output.log` bound to the family-root cycle) and `AuditTrailProjection` (takes `CycleDir`, owns `.cache/rounds/round_NNNN.json` per cycle/fork). Both subscribe to the per-cycle `RunLedger` (`infrastructure/ledger.py`) — every fact (`Decision` / `Phase` / `Snapshot` / `Measurement`) is appended to `events.jsonl` and the projections receive it via the `Projection.on_record` hook. The legacy per-callback API still drives them in parallel; Phase 5 will retire it. Forks are first-class via `RunLedger.inherit_from(parent, offset)`; a fork's `iter()` walks the parent's records up to the cut (including the parent's `FORK_CUT` marker), then its own appends. The `DecisionKind` enum + `DECISION_GATING` table in `domain/run_records.py` are the single source of truth for replayed-vs-archival gating; adding a new kind without an entry fails `tests/test_decision_kinds_registry.py`. The remaining per-cycle audit shape (`index.json`, `log.md`, `trials/`, `langfuse/`, `prompts/`, `archived/`, plus `.cache/candidates/` for resume state) lives in each cycle's own dir. The allowlists (`ROOT_TELEMETRY_ARTIFACTS`, `PER_CYCLE_AUDIT_ARTIFACTS`, `CAMPAIGN_ARTIFACTS`, `SESSION_ARTIFACTS`) live in `tests/test_artifact_parity.py` — the test owns the contract. Entry points MUST NOT write campaign artifacts directly.
