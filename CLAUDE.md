...
- **`grid_search`** — `validate_grid_config()`, `build_grid_combinations()`, `restructure_context()`, `run_grid_search()`, `analyze_grid_results()`, `select_grid_winner()`, `load_eval_dataset()`. Private helpers: `_combo_distance()`, `_allocate_budget()`. Constants: `DEFAULT_GRID_AXES`, `SAMPLING_ALPHA`, `GRID_SEARCHABLE_FIELDS`, `REQUIRED_TEMPLATE_VARS`. `build_grid_combinations()` returns a 3-tuple `(combinations, ps_lookup, sampling_meta)` and supports `n_combos` (exact budget) + `exploration_rate` (0.0–1.0 distance-weighted sampling). `restructure_context()` accepts optional `improvement_areas` for domain-expert guidance and returns a `consultation` key with strategic advice. `run_grid_search()` accepts optional `store`/`backend_id` for content-addressed per-combo caching via `dataset_runs/`, with per-query incremental writes and partial-run resume.
- **`_campaign_lib.py`** — Thin notebook-facing wrapper over the services above. Adds tqdm, print, IPython.display. Preserves legacy `(eval_llm, api_key)` signatures via `_make_llm_client()` adapter. `evaluate_prompt()` supports incremental writes (crash protection) and partial-run resume via `store`/`backend_id` params. `display_progress(campaign_rounds, window=8)` prints training-style progress (accuracy, rolling avg, trend). `run_optimization_loop()` runs semi-automatic optimization with patience-based stopping — subsamples eval_data to `n_samples`, auto-continues while improving, stops after `patience` rounds without improvement.
