...
## TEMPORARY — awaiting an operator decision (delete this section when it lands)

**[`ADR-0002`](docs/adr/0002-identity-foundation.md) § Cross-tenant primitives says the content-addressed caches "stay cross-tenant within a single install". The code does not:** `infrastructure/store/stores.py::build_stores` roots the archive and every `SHARED_CACHE_DIRS` peer at `shared_root / identity.tenant_id`. The four docs that repeated the wider claim are corrected; the ADR is untouched **on purpose — an ADR is a constitution, and whether the primitives were MEANT to pool is a decision, not a doc fix.** Per-tenant looks right on privacy and billing, in which case § Consequences records that they shipped isolated; if pooling was intended, the code diverged from a decision and that is a gap. Two factual amendments ride with whichever answer: § More Information's Stage-1 list names `allowlist.py`, which is now `blocklist.py`, and omits `grants.py`; and the ADR says capabilities "stays empty until a real authorization model is needed" while a seven-rung ladder with sealed file-backed grants is shipped and enforced (`shared/identity.py`'s `CAMPAIGN_*_CAP` ladder and `require_capability`).
