...
**In band, not saturated** — why the model is pinned is owned by the `aime_2025` row of
[`../../docs/operations/dataset-reasoning-matrix.md`](../../docs/operations/dataset-reasoning-matrix.md).
Its limit is **size**: 30 problems cannot carry a config/test split,
so it serves as a head-to-head citation point, never as a population-grade instance.

## Why `max_tokens` is overridden here

`campaign.yaml::pipeline_overlay.llm_only` raises the cap. AIME-depth
