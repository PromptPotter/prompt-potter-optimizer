...
- **The `ForestCard` is NOT one of them — it renders `/tree`, THE served genealogy.** One recursive shape, `course → candidate → (course | sample)`, alternating at any depth: a fork and an L4 inner run are the same edge (a course hanging off a candidate), so L5+ needs no new tier. It is **rooted at a COURSE, not a campaign** (a campaign is a bag of courses; the flat campaign-rooted read it replaced could not describe a leaf below depth 1). Owner: `store/lineage_views.py`, served by `campaigns/cycles.py`; fetched once by `lib/lineage-overlay.tsx`, walked by the one `derivations/lineage-candidates.ts`, laid out by `forest-layout.ts`. **The webapp derives no lineage** — identity (`candidate_id`, `label`) is MINTED on the ledger and served, never re-derived from a list position; there is no client-side tree build and no `round_column_offset`, because a course hangs off a candidate and its columns simply start one right of it. **That genealogy had FIVE witnesses on disk and no owner** — every surface reassembled its own and they disagreed about the same node, which is why a round that never closed lost its candidate (and the finished L4 runs hanging off it) while the ledger held both. Identity comes from the ledger (independent of round close); the round-CLOSE facts (`is_winner`, `theta`, `cumulative_accuracy`) come from `dashboard.json::rounds[]`, which genuinely is the only place they exist. `round_closed` separates a HELD round (closed, crowned nobody) from one that never finished — collapsing those into one `is_winner: false` is how a never-closed round's lone candidate got promoted to a fake winner. **The sidebar reads `/tree` too — through its own `useCampaignCandidates`, deliberately NOT the overlay provider.** That provider is the *viewed* campaign's and carries the What-If / lens / sample-set masks; the sidebar names candidates for any campaign and must show what the run did, not what a mask says it would have done. Folding the two together silently repaints sidebar labels under whatever formula the What-If card is dragging. Both normalize through the one derivation, so a candidate is named and keyed identically on both. The candidates card's bars are the CHILDREN of the VIEWED node in the one tree the sidebar also draws. **Two axes, and keeping them apart is load-bearing:** NAVIGATION is `workspace::viewedPath` + `viewedCandidate` ("which node's children do the bars plot"), written ONLY by the sidebar; INSPECTION is `SelectionContext.candidate` ("which bar is lit"), written by a bar click. They were one slot, which made the chart its own input — a bar click rewrote the state the bars were derived from, so the chart re-plotted under the cursor that clicked it, and the L4 run bars it swapped in are what put `seed-x` where `C0`/`C1.1` had always been. **A bar click never navigates**; the tree is the only navigator. A COURSE plots its candidates (pure `dashboard.json` via the one `roundCandidates` spine; a fork course drops its borrowed C0 — that bar lives on the parent, same rule as its sidebar row) plus one bar per fork cut from it (`derivations/forks.ts::indexForks`, the sidebar's own children-map; served `best_accuracy`, never recomputed). Parking the tree on an L4 CANDIDATE (the sidebar label — the twist expands, the label parks) plots the inner runs filed under it (`inner-panel.ts::cellsForCandidate`, the sidebar's `spawned_by` filing; the title becomes the crumb back up a tier). A plain candidate has no child bars — the bars stay on its course while it scopes the inspector / samples panes. **What a sandbox's cells ARE is `derivations/inner-panel.ts`, once:** root-of-the-sandbox = "is a cell"; `spawned_by` = "which candidate it files under" — two questions, and they part ways on an unfiled cell (listed under the course, never under a candidate). **They are separate cards on purpose:** everything in the candidates card is bound to the chart's box (that's what keeps the dendrogram on its bars), and the forest shares no axis with it, so nesting it there only stole the bars' width.
...
- **One forest read per PATH, not per mount** (`lib/hooks/useForest.ts`). The `/campaigns`+`/cycles` pair answers a whole store at any depth, and four surfaces read the same L4 course's sandbox (sidebar rows, the candidates card's cells, the samples panel, the hard-samples pointer). The state lives in a module store keyed on `encodeCyclePath(at)` — each mount subscribes via `useSyncExternalStore`, and a tick landing inside another mount's `COALESCE_MS` window rides that fetch instead of firing its own. Mount it freely; don't add a second cache or lift it into a provider.
