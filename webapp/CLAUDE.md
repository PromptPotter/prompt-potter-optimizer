...
- **`dashboard.json`** (polled every 2 s by `lib/poll.tsx` → `useCycleStream()`) — in-flight `current_round` and the `rounds[]` array of completed-round summaries (**round 0 = origin**, a one-candidate round labelled "C0"; there is no separate origin block). **Sole source** for the FitnessChart, TrendChart, TopStrip sparkline. Don't stitch in `round_NNNN.json` for chart data.
  - **A live candidate and a closed one are ONE shape** — `DashboardCandidate`, which `RoundSummaryCandidate` narrows to what closing guarantees (id, both fitness numbers, the crown). **Take a WHOLE row from whichever half has it; never fill one in from the other per field.** That merge put a bar on the 2 s dashboard poll and its whisker on the 5 s `/tree` poll, because only `accuracy` had a live fallback. The CI is not a round-close fact — `l1/population.py` stamps it when a candidate finishes scoring — and it travels with **`ci_scale`**, since the election rewrites the band to the θ-accuracy scale only for arms electable on a warm ruler. Both scales coexist in one round, so the whisker resolves its dataset per bar; never re-derive that gate to guess. **A live row's `candidate_id` is POSITIONAL** (`liveCandidateId`) even once scoring stamps a lineage id on it — construction and both live lookups ride that one rule, and a row wearing the other id resolves in neither.
