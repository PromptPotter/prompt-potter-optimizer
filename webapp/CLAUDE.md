...
- **`/tree`** (`lib/lineage.tsx`, one keyed store, one client seam) — the served genealogy: one recursive shape, `course → candidate → course`, alternating at any depth and rooted at a COURSE, so an L4 inner run is a course hanging off the candidate it measured and L5+ needs no new tier. Reach a node with `nodeAt` / `candidatesAtPath`, never a bare cycle_id (inner ids collide across sandboxes) and never a label.
  ...
  - **The SIDEBAR's campaign row reads `/cycles`, not the tree** — it renders before its tree is fetched, so it follows `index.json::status == "rebased_to_fork"` (`grouping.ts::answeringCycle`). Same rule as the tree's `fork_direction`, different evidence; the two part only where one superseded cycle holds several children, which the list cannot tell apart. Ask the tree there.
