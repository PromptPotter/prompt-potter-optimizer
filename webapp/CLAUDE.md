...
- Classify a failure, never bucket it; one condition claims and issues a read → § Failure handling — classify, don't bucket
...
- **Layout is three tiers, decided top-down.** Every file is exactly one of: a **primitive** (`ui/`, `forms/`), cross-surface **chrome** (`shell/` — anything rendering on more than one tab), or part of **one surface** (`chat/`, `dashboard/`, `verify/`, `tree/`=Files, `ingest/`). A file answering two is mis-filed. `components/shell/node-surface/` and `components/shell/searchpoint/` are the worked examples: each renders on several tabs, so filing either under one surface makes the others reach into that folder. **Each host owns only what its own surface can answer** — which cycle is streaming, which round is in flight, which searchpoint is being inspected — and the reading itself takes props. **That licenses IDENTITY props, never SOURCE props:** a host may say *which* campaign, cycle or round to read, and may never say *where* to read it from. Stretched into the second, this rule is what let one node panel render a dataset default on one tab and the running searchpoint on another, because each call site passed a different store (`frontend-surface-contract.md::I9`). Shared app state is a context in `lib/`, never a component; `app/page.tsx` mounts `shell/AppShell.tsx`.
...
- **Never hand-roll a second modal / popover / dropdown / toggle-chip / segmented control / toolbar / clipboard copy / commit-on-blur input.** Reach for `components/ui/*`, or add it there with an RTL test and a `*.module.css` where the thing is presentational at all (`CommitInput` is behaviour, so it carries none).
...
- **A panel that can be READ can be COPIED, and WHAT it copies is a derivation, not the host's call.** `CopyButton` goes wherever a served object is already in scope — no new fetch, no new wire type, or the reach is the bug — but never inside a LABEL: with `choices` it renders an inline `Popover`, so a `<summary>` hosting one takes a floating `role="menu"` into both its content model and its accessible name. Hang it beside the label, not in it. The payload is built ONCE in `lib/derivations` (`searchpointCopyChoices`) and every host of that reading takes it: hosts differ in where the button hangs, never in what a thing IS, and two hosts each serializing a searchpoint their own way is how a paste from one stops matching a paste from the other. Offer several readings only where several are honestly distinct, and DROP the one with nothing behind it rather than copying `null`.
- **An AXIS is a value set, a permitted subset, and a start value — one `ui/ValueList` states all three.** The start value is POSITION 1 (clicking a value pulls it there, and that IS choosing it) and the ticks are the permitted set, so **the lock is DERIVED**: one permitted value is the pin, and a padlock beside it could only contradict it. 🔒 / 🔓 survives where there is no set to narrow — number, string, bool. Closed it is ONE LINE; open, the list drops OVER that line (`Popover side="over"`), so the value is on screen exactly ONCE — as the list's first entry, landing where the closed line sat. A panel hanging BELOW would leave the trigger beside it and print the value twice. A value the operator TYPED wears a `user` tag, which is why `available_models` is served as the ADMIN's catalogue alone: fold their additions into it and the only difference that reads is gone. `model` is an axis like any other; its ticks are the one permitted model set, both what the optimizer may pick and what a human fork may steer to un-tainted. **Each of the three is present exactly when its CHANNEL is** — no `onPick` and the rows are text, no `checked` and the tick column is gone, no `onToggleLock` and no padlock draws. The steer fork passes only the permission channel, because its values are steered on the surface above, and a control that discards its own click is what that absence makes unspellable.
...
- **One served pipeline resolution, addressed by campaign — no call site may choose its own source.** This data class had no row in the table above until the arc that added it, and four stores grew in the gap: the dataset default, a client-held draft, the round file's searchpoint, and `/evidence`'s flattened reading. The tell was one panel answering `upstage/solar-pro4:nitro` on Dashboard and `openai/gpt-oss-20b` on Chat for the same selected campaign, because `NodeDetail` picked its store from whether its *call site* passed a `draft` prop. **A prop may name WHICH campaign/round to read; it may never name WHERE to read from.** Provenance is served per param (`source`), so a badge or a fold computed as a client-side diff against a dataset default is the drift — `frontend-surface-contract.md::I9` is the invariant.
...
**A pending state is a CLAIM that a read is coming — so whatever marks the read started must be the same condition that issues it.** `useDatasetPreview` kept two: an effect marked the slice `started` on the address, while its `load` also required a dataset name that arrives from a later read. On a cold boot it claimed and then silently no-oped, and the `started` ref refused the retry that arrived with the name — so the hard-samples panel reported "loading" over a fetch nobody would ever make, for the life of the tab. `transient`/`invalid` cannot catch this: there is no failure to classify, only a promise nothing will keep. Guard the claim and the fetch with one value, and release the mark in the effect's own CLEANUP — "on every abort path" is not enough while the release is async: cleanup runs before the next effect body, a `load`'s release a microtask after that body has already read the mark and returned, leaving the slice with neither a fetch nor a claim on deps that no longer change.
