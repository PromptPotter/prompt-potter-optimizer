...
**Deep-link carries the WHOLE address — `?path=` plus `?cand=`** (malformed → falls back to following). Both halves, because the address is `(path, candidateId)`: encoding only the path drops the parked node on reload, which for a FORK names nothing at all and leaves the bars blank. Same reason DESELECTING a candidate returns to the course whose TIMELINE it renders on, never to its own path with a null candidate.
