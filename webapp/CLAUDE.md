...
- **Never hand-roll a second modal / popover / dropdown / toggle-chip / segmented control / toolbar.** Reach for `components/ui/*`, or add it there with a `*.module.css` and an RTL test. **Nor a second pipeline GRAPH:** every one, on every surface, is `dashboard/pipeline/PipelineFlow` over the served `view` + its `tier`/`rank`. A bespoke hand-placed geometry sat beside it for exactly as long as it took to disagree — it drew three edges the wire never declared and dropped the loop and both escalations, on the two surfaces that were NOT it.
...
- **A non-item is not the same as discarded.** `sample_order_preview` yields no item — nothing *happened*, it is the order the scorer is about to walk — but the stream is its **only** channel, since `LiveDashboardView` never persists it. It is read as STATE beside the feed and drives the run card's "next in line". A *declared* order, never a promise: PoBB can stop a candidate before its tail is reached, so no surface may word it as "will".
