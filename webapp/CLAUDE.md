...
- **`dashboard.json`** (polled every 2 s by `lib/poll.tsx` → `useCycleStream()`) — in-flight `current_round` and the `rounds[]` array of completed-round summaries (**round 0 = origin**, a one-candidate round labelled "C0"; there is no separate origin block). **Sole source** for the FitnessChart, TrendChart, TopStrip sparkline. Don't stitch in `round_NNNN.json` for chart data.
  - **A live candidate and a closed one are ONE shape** — `DashboardCandidate`, which `RoundSummaryCandidate` narrows to what closing guarantees (id, both fitness numbers). **Take a WHOLE row from whichever half has it, and choose the half ONCE; never fill one in from the other per field.** That merge put a bar on the 2 s dashboard poll and its whisker on the 5 s `/tree` poll, because only `accuracy` had a live fallback. **The half is the TREE unless it holds no measurement for that candidate at all** (`CandidatesCard::views`) — the ledger mints a candidate before it measures one but only snapshots the score at completion, so a bar mid-scoring is the single thing the tree cannot answer. It used to prefer the live row for the whole of an open round, because the crown rode the round's close record; the election has its own record now, so that window is gone.
  - **Neither the crown nor the whisker is a round-close fact.** `elect_round_winner` is the last thing `l1_score` does, so `is_winner` is decided at the end of SCORING — a whole `l1_critique` call before the round closes — and it therefore lives on the BASE shape, not the closed-round narrowing. The CI likewise: `l1/population.py` stamps it when a candidate finishes scoring. **ONE band per candidate, one writer, no scale field** — it hangs off the accuracy bar for every arm. It was briefly two estimators sharing one field pair with a served `ci_scale` to tell them apart, and because only arms an election reached were ever relabelled, the whisker appeared and vanished by gating rather than by evidence. Don't re-add a per-bar scale; keep the one band meaning one thing. **A live row's `candidate_id` is POSITIONAL** (`liveCandidateId`) even once scoring stamps a lineage id on it — construction and both live lookups ride that one rule, and a row wearing the other id resolves in neither.
  - **"Which optimizer node is running" is served** (`current_round.active_node`), resolved at the writer over the full `DashboardState` vocabulary. Don't reconstruct it from `in_flight.node` + `state` — the partial version of that map is what made the canvas go dark between two calls in one phase. There is likewise **no `live` flag**: "is the run running" is `run_phase` + poll freshness, and "is this the round I'm looking at" is `current_round.round`. `current_round.nodes` is filtered to that round server-side, so presence in it *is* "this node has fired here".
- **The `ForestCard` and the sidebar are NOT `dashboard.json` — they render `/tree`, THE served genealogy, and the webapp derives no lineage.** One recursive shape, `course → candidate → course`, alternating at any depth, rooted at a COURSE: an L4 inner run is a course hanging off the candidate it measured, so L5+ needs no new tier. Owner `store/lineage_views.py`, read through **one** client seam (`lib/lineage.tsx`) over one keyed store. Identity (`id`, `label`) and everything a candidate's ROUND decided about it (`is_winner`, `election_held`, `theta`, `cumulative_theta`) fold from the cycle's own LEDGER — never read back out of `dashboard.json`, which is a projection. A round crowns nobody when it never elected OR when a correction retired its winner, so `pickWinner` has no first-candidate fallback and must not grow one. **The crown is an ELECTION fact and rides the election's own record**, so the tree crowns the moment `elect_round_winner` decides — a whole `l1_critique` call before the close. **θ is not**: it is re-fit when the ruler warms, which is what round 0's second close exists to deliver, so it stays on `round:complete`. Moving either half to the other record raises nothing and breaks one of them silently. **`is_winner: false` says nothing on its own** — a round that HELD reads exactly like one still scoring — so `election_held` is served beside it and a surface asks THAT, never whether the round has closed.
  ...
  - **A cut that moved the line means the BRANCH answers for the campaign** — the course row's `run_phase` / `status` / `best_accuracy` are served from it, because the parent's own row describes a retirement (`rebased_to_fork`, stamped at the cut) while the cycle that IS running owns no row of its own. `supersede` AND `equivalent` both move it; only an `offshoot` leaves the parent running. That is the whole reason a course row can show a green ● at all: a fork is not a node, so without this nothing in the tree carries `running`. **The SIDEBAR's campaign row applies the same rule from the other side** — it renders before its tree is fetched, so it reads `/cycles` and follows `index.json::status == "rebased_to_fork"`, the stamp `mark_superseded` writes and nothing else does (`grouping.ts::answeringCycle`). Same rule, different evidence: the tree has `fork_direction`, the list has the retirement the cut left behind. They part only where one superseded cycle holds several children, which the list cannot tell apart — ask the tree there.
