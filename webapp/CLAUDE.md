...
- **`llm_call_progress` rides the ray on purpose.** The client uses a bare heartbeat to prove the process was alive across a silent stretch and then drops it from the rendered steps (`projectionToActivity` already returns null for one). Stop sending them and every heartbeated backend query grows a spurious gap marker. The coupling is commented at `format.ts::fmtGap`, `derivations/time-ray.ts` and `store/family_ray_views.py`.
