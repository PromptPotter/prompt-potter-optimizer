...
- **`dashboard.json`** (polled by `lib/poll.tsx` → `useCycleStream()`) — the in-flight `current_round` block and the `rounds[]` array of completed-round summaries. **Sole source** for the FitnessChart, TrendChart and TopStrip sparkline; don't stitch in `round_NNNN.json` for chart data.
  ...
  - **ONE band per candidate, and no scale field.** It hangs off the accuracy bar for every arm. Don't re-add a per-bar scale; keep the one band meaning one thing.
