...
PromptPotter is **LLM-driven program evolution** for prompts and pipeline params. CLI-first today; the file tree (per-cycle `dashboard.json`, `rounds/`, `log.md`) is the operator's primary surface, alongside a webapp control plane served at the **domain root** (Next.js static export at `webapp/out/`, source at `webapp/`) that polls `dashboard.json` and issues control verbs via `POST /commands/{kind}` (M12), keeping the file tree authoritative. Whitelabel distribution is a stated goal — every brand element must be themable.
...
5. **Whitelabel-safe by default.** Accent color, mark, and brand copy flow through tokens (`--color-accent`, `--brand-name`). Never hardcode "PromptPotter" into a styled element that resists rebrand. **What a distributor sets, and the four tiers of rename** — owned by [`deploy-linux/README.md`](deploy-linux/README.md) § Running it under your own name; a new brand element here needs a declaration there, or it is unreachable.
...
## Replacing the mark (whitelabel)

**The mark has no env override, because a glyph is not a string** — a distributor swaps the artwork instead. Three files, and nothing else refers to them: `mark-pot.png` (login eyebrow, running-jobs button, About-this-unit), the `tab-icon-pot-32` ink/bone pair, and `app-icon-pot-512.png`. Their constraints are § Mark lineage's — **alpha is the mark**, the tab pair is two files because raster cannot carry the media query, and the app icon must be opaque. Two additions for a replacement:

- **Do not wrap it in a circle to make it survive both themes.** The tab pair already solves that, and only for the tab.
- **`SurfaceFavicon.tsx`'s gold/ink grounds have no override yet** — a distributor's tab still says *which install* this is in our colours.

Everything else a distributor sets → [`deploy-linux/README.md`](deploy-linux/README.md) § Running it under your own name.
