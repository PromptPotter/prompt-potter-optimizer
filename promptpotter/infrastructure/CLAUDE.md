...
any write, answer whether they still can — and a SERVED read rests on the same guarantees
(`cell_queries::cycle_cells` reads the round in flight off this file). It is **always on disk and always swapped atomically**
...
and no field on this model is on-disk shape anyone must migrate. For the readers that DO parse the
file (the model is `extra="forbid"`), the prior state is dropped WHOLE and loudly, never salvaged
field by field — a partial read is a compatibility shim, and the ledger is the truth. The SSE
...
**`measurements/` is ONE content-addressed tree per workspace, and it outlives the campaigns that filled it.** Three consequences, each easy to read backwards:
...
**No paid request is sent unadmitted, and a usage record is only ever a BILL** — the hold / bill /
unreported rules are `llm/spend_book.py`'s header. Every paid path goes through it:
`LLMClientBase._admitted_send`, Harbor's in-process agent send by send (`llm/litellm_sends.py`, so
its cell only RESERVES the run it DECLARES), and `BackendClient.run_query` for a remote cell. SDK
retries are off — each resend (`llm/rate_limit.py`) is admitted anew. A caller meters
...
This note sits here rather than only in `mlflow_sink.py`'s docstring because the subtree reads as dead code to every sweep that measures deadness by counting readers. If it is *badly written*, refactor it; absence of a reader is not the reason.
