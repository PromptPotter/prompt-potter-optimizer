"""Active session pointer — survives across CLI invocations.

Payload shape is ``{tenant_id, session_id, cycle_id}``; ``backend_id`` is
*not* part of the pointer because it's no longer the project axis.
Callers that need the backend_id read it from the session's state blob.
"""
