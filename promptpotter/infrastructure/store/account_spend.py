...
if TYPE_CHECKING:
    # Type-only: this module reads the campaign store's own ledgers and lives beside it, so a
    # runtime edge would close the loop.
    from promptpotter.infrastructure.store.campaign_store.store import CampaignStore


def account_ledgers(campaigns: CampaignStore) -> list[Path]:
    """Every file an account's lifetime spend is recorded in. It takes the STORE rather than a
    ``Stores`` so the cross-tenant install report sums by the same walk, over a store it rooted
    itself."""
    return list(campaigns.iter_cycle_ledgers())
...
def sum_user_spend(*, ledgers: Iterable[Path], since: float, until: float) -> UserSpend:
    """Both units plus the unpriceable residue — the second is what the first cannot see when no
    rate is on file."""
    used_usd = 0.0
    used_tokens = 0
    unpriced_tokens = 0
