"""``log.md`` digest renderer + hard-sample heatmap.

Pure functions over disk-loaded dicts (``index.json``, trial JSONs,
hard-sample artifacts). No I/O, no global state, no terminal-display
primitives — markdown / fenced-text only. Lives in ``application/`` so
the campaign runner can render the per-cycle log without crossing into
``presentation/``.
"""
...
def render_log_md(
    ...
    *,
    ...
    parts += [
        ...
        f"- baseline: {_fmt_pct(index.get('baseline_accuracy', 0.0) or 0.0)}",
        (
            f"- best: {_fmt_pct(index.get('best_accuracy', 0.0) or 0.0)}"
    ...
    for trial in trials:
        ...
        parts += [
            f"### Round {rnd} — {label} ({_fmt_pct(trial.get('accuracy', 0.0) or 0.0)})",
            "",
