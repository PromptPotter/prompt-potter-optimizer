...
- `origin.py` — campaign origin scoring + dataset loading. `resolve_origin_opt_search_point` resolves the origin OSP by priority **seed → dataset prompts → empty**: a `CycleSeed`'s `origin_prompt_fields` (read from the cycle's `CycleSeedRecord` on the ledger) *is* the origin (operator-steered fork or campaign-from-origin; lineage stamped from `seed.origin_source`). It is the ONE origin recovery — `verify`'s C0 reading calls it rather than reconstructing prompt fields off round 0's row, because a second recovery is a second origin.
- `embedded_run.py` — the **embedded launch entry**: `open_session` → `run_campaign`, plus `submit_gate_decision` (the gate has no TTY here and `origin_gate` defaults `strict`, so a host that cannot answer blocks at round 0 forever), for a host Python program driving one campaign in its own event loop (the notebooks, the BBEH harness, `scripts/smoke_campaign.py`, a packaged adapter). Peer of `jobs/launcher/mint_and_start.py`, which detaches onto a background task and takes a machine slot or queues for one; this one blocks in the caller's loop and **deliberately holds none** — the host program owns its own scheduling, and a library call that took a slot out of a shared pool would make a notebook wait on a browser. That is a declared exemption from `request_launch`, not a missing call: it is the one launch entry whose caller is another program. Two steps rather than one because every caller does its own work between them; the origin is scored inside `run_optimization`, so a stop there closes the cycle as it does for every entry point. It takes a `LiveDisplay` and an `on_status` callback but renders neither — the read-out of what it returns is `presentation/terminal/completion.py`.
- `run_observers.py` — `RunCallbacks` typed event constructor over `CycleEventLog.append`. Also binds the `_CYCLE_LEDGER` ContextVar (`build_run_observers`) — anything reaching an LLM *before* this runs must bind its own ledger or its spend goes unrecorded. **And it owns `build_campaign_emitter`**, the `LiveDashboardProjection` constructor: it sat in `origin.py`, which builds no projections and whose importers had to reach past their own concern to find it.
...
- **Per-call telemetry from deep async chains uses the `emit_*` shape**, not
  ...
  `Projection` (`infrastructure/projections/base.py`), write a kwargs-only
...
**Two moments read that schema, not one, and the second is the one that gets forgotten.** Run init reads it live (`initialization/wiring.py`); the CHECK-IN reads it too (`datasets/ingest.py::fetch_backend_nodes`, stored on `DraftCampaign.backend_nodes`) because `optimizer.param_keys` — which params are search AXES — is declared by the backend and by nothing else. A setup surface built from the connector seed alone concludes nothing is movable and draws locks the run does not enforce. Store it on the draft rather than fetching per response: it is a material fact about that check-in, and an unreachable backend must surface as `schema_source: unreachable` rather than as an axis nobody opened.
