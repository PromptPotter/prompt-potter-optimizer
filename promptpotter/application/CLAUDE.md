...
| `scoring/` | The `score_search_point()` gateway plus formula compilation, evaluators, sample measurement, and three modules that were one bag named `metrics.py`: `metrics.py` (the composite-fitness COMPUTER — the gateway is not it), `selection.py` (which candidate wins / is cut: `elect_round_winner`, `elimination_p_best`, `paired_fitness`, `composite_ci`), `diagnostics.py` (what one measured row REPORTS: `find_rank`, `extract_sample_diagnostics`). Per CLAUDE.md: gateway is canonical; everything else is implementation detail. |
