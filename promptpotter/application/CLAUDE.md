...
- `optimizer_prompt_ranking.py` — ranks optimizer prompt states by anchor-to-origin paired effect, recomputed from disk per read (`GET /optimizer-prompt-ranking`). **Read-only, and deliberately so** — it names a leader; graduating one into `promptpotter/assets/optimizer/pipeline.yaml` is a hand-edit. The CLI verb `rank-optimizer-prompts` reads it and writes nothing; an earlier verb of the same shape that DID write that file was deleted, so treat that manifest as operator-owned. It also serves `OuterSnr` — whether the ranking can be believed at all (L4 finish-line item 7), pooled from the same walk: the noise series is **k replicate readings within ONE occurrence**, the spread of `anchor_effect` across states the signal. **Never a union across occurrences** — the inner instrument is content-addressed, so a second campaign on the same (state, cell) replays rather than re-reads, and pooling those yields a spread of zero. `None` rather than a number wherever the corpus cannot support one, including a σ of exactly 0.0. A leader from a panel that cannot resolve arms is noise wearing an ordering.
