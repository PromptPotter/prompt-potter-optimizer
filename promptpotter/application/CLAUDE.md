...
- `sweep_batch.py` — one fork per `OperatorSweepFile` via the shared `_mint_fork`. Reached from CLI `new --sweep-batch`; the second, hand-rolled `sweep` verb that did the same job is gone. The generation-only round each fork ends on is `runner/generation_only.py`, shared with `--diag` — which is why it is named for the `status` it writes, not for either caller.
