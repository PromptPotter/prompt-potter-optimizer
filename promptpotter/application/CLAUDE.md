...
- `evidence.py` — what ANY SET of SUBJECTS jointly says, recomputed from disk per read (`GET /evidence`, CLI `evidence --subject`). **Read-only**: it names a leader, and graduating one is a hand-edit. Eight rules, none of them readable off the code:
...
- `sweep_batch.py` — **DEPRECATED, removal pending; add nothing to it.** A contrast is declared as `axes:` in a dataset's `inner_tasks.yaml` and read back with `evidence --grid`: that reaches DATASET, which a fork cannot (they share one campaign), and generates a balanced product, which a hand-listed roster does not. One fork per `OperatorSweepFile` via the shared `_mint_fork`, reached from CLI `new --sweep-batch`. The generation-only round each fork ends on is `runner/generation_only.py`, shared with `--diag` — which is why it is named for the `status` it writes, not for either caller.
...
- `verify.py` (the `verify` verb), `noise_floor.py` (the fenced `noise-floor` diagnostic — never wired into the loop), `ab.py` (the `ab` verb), `run_phase_control.py`.
