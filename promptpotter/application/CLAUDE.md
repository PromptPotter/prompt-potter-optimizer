...
- `dataset_evidence.py` — what EVERY campaign on one dataset jointly says, recomputed from disk per read (`GET /datasets/{name}/evidence`, CLI `dataset-evidence`). Two halves. The **origin half answers at round 0**, which is where a campaign usually stops and is therefore the case that kept going unserved: the roster, `levels_comparable` (every origin on one known `ruler_id` — `None` is UNKNOWN and must never read as yes), the arm replicates, the cell/arm/residual decomposition beside the null scatter an arm mean shows anyway, the resolving power, and the run-order confound. The **ranking half** needs a scored L1 round: each state's anchor-to-origin paired effect with its own interval, plus `OuterSpread` — how far apart those effects are. **Read-only, and deliberately so** — it names a leader; graduating one into `promptpotter/assets/optimizer/pipeline.yaml` is a hand-edit, and an earlier verb of the same shape that DID write that file was deleted. **There is deliberately no noise term beside the spread:** a second reading of one (state, cell) replays rather than re-measures, because the inner instrument is content-addressed, so its spread is zero by construction and reads as a perfect instrument. A replicate ARM is the honest noise reading, and depth on one candidate is `verify`'s job — more samples, not the same question twice. A leader read off the ordering rather than off its own interval is noise wearing a rank. Everything is in the measurand's own units, never composite fitness: fitness is what each campaign's own `scoring` formula made of the measurand, so pooling on it averages numbers from different formulas.
