...
def subject_evidence(
    ...
    *,
    ...
    levels_by_subject = _levels_by_subject(rows, {k: _config_of(h.point) for k, h in heads.items()})
    rows = [r.model_copy(update={"levels": levels_by_subject[r.key]}) for r in rows]
    ...
    return Evidence(
        ...
        factors=_factors(rows, levels_by_subject, spec_metric),
        grid=_grid(rows, levels_by_subject, spec_metric, grid) if grid else None,
...
def _factors(
    ...
    levels_by_subject: Mapping[str, dict[str, str]],
    spec: MetricSpec,
) -> list[FactorReading]:
    ...
    keys = sorted({k for lv in levels_by_subject.values() for k in lv})
    ...
    for key in keys:
        ...
        for subject, levels in levels_by_subject.items():
            buckets.setdefault(levels[key], set()).add(subject)
    ...
    for key in keys:
        ...
        for subject, levels in levels_by_subject.items():
            grouped.setdefault(levels[key], []).append(subject)
...
def _grid(
    ...
    for subject, levels in levels_by_subject.items():
        grouped.setdefault((levels[row_key], levels[col_key]), []).append(subject)
