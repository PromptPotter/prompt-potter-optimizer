...
* ``l1_strict`` — STRICT schema-validation surface for ``l1_generate``
  output. Builds per-pipeline JSON Schemas, normalizes pipeline_params
  overrides, runs ``detect_invariants`` over yield stats. Failures route
  through L2 as ``ValidationFailure``.
* ``l1_behavior`` — SOFT behavior checks (load-bearing per
  ...
  it's structural: ``node_param_keys`` never emits the axes, with the
  ``l1_strict.validate_overrides`` backstop.)
