...
`dispatch/injections/` composes most of every optimizer prompt, so rewording a directive changes what every inner cycle is handed — and on L4 that is measurement identity. `injection_source_digest` covers the three things that decide what a node is handed — the renderers, `bundle` (how much of a panel arrives) and `compose` (which panels arrive at all) — AST-normalized: a comment, docstring or reflow costs nothing; a panel's prose, its `char_cap`, a node ceiling or a selection rule voids the banked origins. Expect the re-measure; the bug is the reverse. **A prompt-shaping constant belongs in `bundle.py`** for that reason: `OPTIMIZER_PROMPT_BUDGET_CHARS` sat in `config/settings.py`, outside the hash, and the commit that introduced it dropped nine of `l1_generate`'s evidence panels from most of its prompts while voiding nothing. **The wire schema's constants live there too, and it is the only place they can** — `l1_wire_schema` reaches `injections/registry` through `llm_call/prompts`, so hashing that module closes an import cycle. Its emitted JSON Schema is prompt text the ceiling above does not even count, so what it declares — a `maxLength`, a param description — must sit in `bundle` or it changes every prompt for free.
