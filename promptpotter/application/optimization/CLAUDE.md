...
- **The `*_override` slots derive the same way**: a slot whose panel produced nothing this call is withdrawn. L1 may not EDIT an axis it was never shown any more than it may cite one — nor a prompt field the operator held: `prompt_fields_updates` offers `open_prompt_fields()` alone, the one carrier of those fields.
