...
- `mutation_memory` derives from the payload, **never** from `changes_description` — optional prose two candidates can share. A PoBB-cut candidate has no matched parent at all (`scoring/metrics.py::matched_parent_stats`): it ran a prefix of an order stratified on the incumbent's own grades, so the shared cells ARE the incumbent's misses and both a partial accuracy and a paired win/loss would be decided by where it stopped rather than what it did. Its subset-invariant standing is the θ beside it.
