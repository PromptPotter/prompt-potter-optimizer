...
convention: `TaskDecomposition.merge` refuses them and the L2 wire schema has no field for
...
**`terminate_proposal`** stops the cycle outright on the existing HALTED-class `StopReason.ABORT` — no new stop reason, no fork. Its canonical user is an **evidence-starved** node: an enricher failing across the round because its backend quota is exhausted. That grade *routes* L2 in via the `l1_evidence_starved` rule — **a weak preemptor that brings L2 to diagnose and never stops anything itself** — and L2 judges recoverability, which a deterministic rule cannot and the LLM tier can. So stop authority lives here rather than in a backend-coupled tripwire, and what the operator acts on is the `suggested_action` banner carrying the verbatim connector reason.
...
- **Task decomposition** — CLI `new`. Raw `task_description` → the six Layer-1 prompt strings (`persona`, `task_intent`, `problem_description`, `instruction`, `thinking_style`, `answer_format`) + the `task_context` sub-object. Driver: `task_context.py::decompose_prompt_fields`. **Check-in is the SOLE producer of framing, and it commits it as dataset content** (`task_context.yaml`) — nothing decomposes at run time. The framing renders (it splices around `problem_description`), so `build_origin_cycle_id` hashes it; a run-time decomposition would land after the id that must name it. One pure reader, `committed_task_context`, serves identity, C0, `seed-screen` and the notebook alike.
