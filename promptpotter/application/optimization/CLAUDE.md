...
The cycle has hardcoded stop conditions (e.g. `max_rounds`). On top of those, **the L2-layer and L3-layer may end the cycle themselves** — either through the deterministic escalation rules (goal reached / infinite stall) or by emitting a `terminate_proposal` when the fault is unrecoverable through any framing or plan move (the LLM-emitted stop; see the layer-control channel below).
