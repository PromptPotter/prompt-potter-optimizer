...
| `PromptTemplate` | `search_point.py` | 8-field prompt scheme with `render()` / `compile_prompt()`. Canonical prompts at `datasets/{name}/prompts/{node}.json`. |
...
| `DecisionKind` + `DECISION_GATING` | `run_records.py` | Import-time exhaustiveness — adding a kind without a gating mode raises before the module loads. SoT for replayed-vs-archival gating. |
