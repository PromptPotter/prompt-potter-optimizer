...
| `AbilityReading` | `ruler.py` | **A θ and the δ scale it was read on, as ONE value** — `theta` / `se` / `ruler_id` / `ruler_n` / `calibration_model`, none defaulted, so no producer can stamp a level and leave its scale to be inferred. `comparable_to` is the only sanctioned test for whether two θ may be differenced (same `ruler_id`, and never where either is `None` — that names NO scale); `scale()` is the single rendering, so no surface reassembles the words. It exists because those five travelled as loose fields and each consumer re-declared a different SUBSET of them — the export carried three, so a banked θ named no ruler at all. |
