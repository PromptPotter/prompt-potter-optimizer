...
| `OptSearchPoint` | `opt_search_point.py` | Optimizer state: the 6 decomposition fields + `few_shot_examples` + `plan` + `lineage` + `memory: L2L3Memory` (wounds / l1_layout / l1_overrides / task_context) + `plan`. **All new optimizer state flows through here** — no sidecar state. |
