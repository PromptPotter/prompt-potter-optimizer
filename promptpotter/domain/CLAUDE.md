...
| `ForkSpec` / `OperatorForkOverride` / `LimitOverrides` | `run_records.py` | The one typed fork record + its operator-steer payload (domain twin of the wire `OperatorForkOverride` schema). Every operator fork is `operator_steered` and carries an `OperatorForkOverride` (`{origin_prompt_fields, pipeline_overlay, limit_overrides}`); non-operator triggers (sweep / diag / rebase) carry no seed. One writer (`_mint_fork`), three projections (`FORK_CUT` ledger SoT, `index.json::fork`, `.overrides/seed.json`). |
