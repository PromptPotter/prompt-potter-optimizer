...
| `ForkSpec` / `ForkSeed` / `LimitOverrides` | `run_records.py` | The one typed fork record + its operator-steer payload. Every operator fork is `operator_steered` and carries a `ForkSeed` (`{origin_prompt_fields, pipeline_overlay, limit_overrides}`); non-operator triggers (sweep / diag / rebase) carry no seed. One writer (`_mint_fork`), three projections (`FORK_CUT` ledger SoT, `index.json::fork`, `.overrides/seed.json`). |
