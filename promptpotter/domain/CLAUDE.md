...
| `ResumeCheckpointKind` + `RESUME_CHECKPOINT_GATING` | `run_records.py` (kind enum) + `application/optimization/resume_and_fork/decisions.py` (gating dict) | Import-time exhaustiveness — adding a kind without a gating mode raises before the module loads. SoT for replayed-vs-archival gating. |
| `ForkSpec` / `CycleSeed` / `LimitOverrides` | `run_records.py` | The one typed fork record + the chosen-searchpoint seed a non-root cycle begins from (`{origin_prompt_fields, pipeline_overlay, limit_overrides, origin_source}`). Every operator fork is `operator_steered` and carries a `CycleSeed` (the wire `OperatorForkOverride` command payload deserializes into it); the mint seam writes one for campaign-from-origin; non-operator triggers (sweep / diag / rebase) carry no seed. `origin_source` (`fork_seed` \| `campaign_origin`) stamps the C0 lineage. For forks: one writer (`_mint_fork`), three projections (`FORK_CUT` ledger SoT, `index.json::fork`, `.overrides/seed.json`). |
