"""Format-only helpers for per-candidate + per-sample CLI/notebook rendering.

- ``fmt_pp_override`` flattens a nested pipeline-params dict to a single line.
- ``fmt_individual_header`` renders the pre-scoring per-candidate header.
- ``_fmt_query_result`` formats a single HIT/MISS query line plus
  diagnostic annotations for ``LiveDisplay.on_sample_scored``.

The dict→typed-summary factory (``individual_summary_from_dict``) and the
``IndividualSummary`` dataclass live in ``view_factories.py`` alongside the
other ``*_from_dict`` factories. Round-summary renderers (progress table,
round stats, patience banner) live in ``live.py`` because they have a
single caller there.
"""
