"""Live-listener event renderers — pure functions over event/result dicts.

Consolidates per-query, per-candidate, and per-round renderers that the
``LiveDisplay`` listener (and, for ``render_progress_table``, post-hoc
CLI ``show-*`` commands) call to turn structured data into a printable
string. Zero domain-model mutation, zero ANSI assembly — primitives are
pulled from ``display_primitives`` and column/box helpers from
``formatting``.

Sections:

1. Per-query: ``_fmt_query_result`` — HIT/MISS/DEPR line for one sample,
   plus warning/rerun annotation lines underneath.
2. Per-candidate: ``IndividualSummary``, ``build_individual_summary``,
   ``fmt_individual_header``, ``format_elimination_summary``,
   ``fmt_pp_override`` — single source of truth for what each display
   shows per candidate.
3. Per-round: ``render_progress_table``, ``render_round_stats``,
   ``render_patience_status`` — round-boundary report block.
"""
...
from .formatting import fmt_ci, fmt_pvalue
