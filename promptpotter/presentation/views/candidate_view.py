"""Candidate-card render kernel — shared between CLI and notebook displays.

Both entry points classify + format the same way; only the outer wrapping
(flat ``tqdm.write`` vs box-drawing) differs. This module owns:

- ``fmt_pp_override`` — flatten nested ``pipeline_params_override`` to a
  ``node.key: val`` line.
- ``fmt_candidate_header`` — pre-scoring one-liner emitted from
  ``on_candidate_started``.
- ``format_elimination_summary`` — Wilcoxon-elimination one-liner.
- ``CandidateSummary`` + ``build_candidate_summary`` — single source of
  truth for what each display shows per candidate.
"""
...
from .display_primitives import CYAN, DIM, RESET, YELLOW, _fmt_delta, _pp_val
