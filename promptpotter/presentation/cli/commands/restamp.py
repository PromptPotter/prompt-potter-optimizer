...
async def cmd_restamp(args: argparse.Namespace) -> CommandResult:
    ...
    refiled = refile_round_decisions(apply=apply)
    ...
    human = (
        ...
        f"Round documents re-filed: {refiled['round_files']}."
    )
    return CommandResult(data={**counts, **ledgers, **refiled}, human=human)
