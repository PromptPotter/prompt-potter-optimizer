...
- **The webapp test pool is bounded to four workers** — unbounded, Vitest forks `availableParallelism() - 1` (15 on this box), each paying its own jsdom setup, and one starved its own startup while `gate.py` ran four checks at once. Cumulative environment setup 51.7s → 20.9s. Four is the gate's own concurrency; it costs ~8s standalone and nothing in the gate, where eslint is the critical path at 48.0s and this check finishes inside its shadow.
