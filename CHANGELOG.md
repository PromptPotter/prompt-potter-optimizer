...
- **`python scripts/gate.py` is 27.8s, not 50.3s** — it was re-doing work it had already paid for, and 12 of 16 cores sat idle while it did. Four things, and the order was set by measuring rather than guessing: **eslint had no cache at all** (`--cache --cache-strategy content`; 48.0s → 14.8s in-gate, 4.4s standalone warm — `content` over the default because commits are verified in fresh worktrees, where every mtime is new and an mtime cache misses wholesale); **pytest ran single-process** on a suite with no session fixtures, no network and one `tmp_path` fixture (`-n`; 42.1s → 18.7s); **tsc kept two incremental stores and warmed neither**, because the staged mode passed its own `--tsBuildInfoFile` while `tsconfig.json` already sets `incremental` (20.2s → 8.5s, and the second store is deleted); and **`npx` re-resolved a package per call**, so both node checks go through `package.json` scripts — which is how `typecheck` came to exist, since the fast invocation had been hiding inside the gate. **The wall moved twice on the way**: eslint hid pytest 6s behind it, and fixing pytest handed the wall to vitest. So the verdict line now names the slowest check — the total is `max(slowest, sum/pool)`, and a green run that reports neither is a number nobody can act on.
- **One parallelism budget, divided** — three things each independently assumed they owned the machine: the gate's pool, Vitest (`availableParallelism() - 1`), and Turbopack (every core, and `next build` runs beside three other checks). That contention is most of what the gate's per-check timings were measuring — eslint costs 4.4s alone and 14.8s in-gate. `gate.py` declares `_JOBS` / `_POOL` / `_SLICE` once and hands each parallel check its slice, so peak demand is bounded by construction instead of inferred in three places; the build's cap rides the `DEPLOY_BUILD` env channel it already had, so the operator's own preview build still gets the whole box. CI restores `.eslintcache`, because **writing** a cache costs more than not having one (~15s against ~10s cold, ~4s warm) and a runner that never restores it would pay the write every run and collect the read never.
- **`npm run dev` is the documented loop for visual work** — HMR at :3000 against a rebuild that prerenders every route, because `output: "export"`. The `/api/*` rewrite in `next.config.ts` exists for this and nothing else, and was reachable only from a single parenthetical. The port picks the harness: :3000 covers anon and `PROMPTPOTTER_AUTH=off`, while the OIDC redirect is bound to :8001, so a real login round-trip still needs the built app. `webapp/README.md` was create-next-app boilerplate — wrong about the package manager, the fonts and the deploy target — and is now a pointer to the owner.
- **`composite_ci_*` is `mean_fitness_ci_*`, and `matched_origin_*` is `matched_parent_*`** — both names lied about what they hold. The interval brackets **per-cell mean fitness** whatever the composite formula says, and the thing it matches against is the round's `RoundParent` — the origin only at round 0, the prior winner after it. 351 occurrences over 60 files; the served API fields, the generated TypeScript and the round document move with them. **Start clean:** `ScoredCandidate` is `extra="forbid"` and `RoundResult`'s `extra="ignore"` does not reach a model nested inside it, so every existing round document is hard-fatal on read — 399 of them were wiped rather than migrated, which the content-addressed measurement archive makes free (it replays by `sample_id`, so nothing is re-paid).
- **The webapp test pool is bounded to four workers** — unbounded, Vitest forks `availableParallelism() - 1` (15 on this box), each paying its own jsdom setup, and one starved its own startup while `gate.py` ran four checks at once. Cumulative environment setup 51.7s → 20.9s. The bound stays as the standalone `npm run test` default; under the gate the shared budget above decides, because there it shares the box.
...
- **`reset` could not see half the disk** — `.inner/` is a *sibling* of `projects/`, so the L4 sandboxes were neither dropped, nor preserved, nor reported as unrecognized, in a verb whose contract is that it "never reaches a path it cannot explain". 146.8 MiB over 3,190 files survived a wipe that took a 1.3 MiB `campaigns/` tree — and an inner campaign id is content-addressed, so the next `new promptpotter-self` would have found those keys already present and **continued** the stale sandboxes instead of starting clean. Each one now drops through `delete_inner_sandbox`, which banks its spend before taking the rows; a sandbox whose `owner.json` cannot be attributed to a tenant is kept and named, never guessed at. `--dry-run` lists what it would take.
